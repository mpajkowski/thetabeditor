...
void SystemRenderer::centerHorizontally(QGraphicsItem& item, double xmin, double xmax)
{
...
{
  item.setY(y + 0.5 * (LayoutInfo::SYSTEM_SYMBOL_SPACING - item.boundingRect().height()));
}
...
QGraphicsItem* SystemRenderer::operator()(System const& system, int systemIndex)
{
  ...
  const ViewFilter* filter =
    myViewOptions.getFilter() ? &myScore.getViewFilters()[*myViewOptions.getFilter()] : nullptr;
  ...
  for (Staff const& staff : system.getStaves()) {
    ...
    LayoutConstPtr layout = std::make_shared<LayoutInfo>(myScore, system, systemIndex, staff, i);
    ...
    myParentStaff =
      new StaffPainter(layout, ScoreLocation(myScore, systemIndex, i), myScoreArea->getClickPubSub());
    ...
    // Draw the clefs.
    const double CLEF_OFFSET = (staff.getClefType() == Staff::TrebleClef) ? -6 : -21;
    auto pubsub = myScoreArea->getClickPubSub();
    const ScoreLocation location(myScore, systemIndex, i);
    auto clef = new SimpleTextItem(staff.getClefType() == Staff::TrebleClef ? QChar(MusicFont::TrebleClef)
                                                                            : QChar(MusicFont::BassClef),
                                   myMusicNotationFont);
    auto group = new ClickableGroup(QObject::tr("Click to change clef type."),
                                    [=]() { pubsub->publish(ClickType::Clef, location); });
    group->addToGroup(clef);
    group->setPos(LayoutInfo::CLEF_PADDING, layout->getTopStdNotationLine() + CLEF_OFFSET);
...
void SystemRenderer::drawTabClef(double x, LayoutInfo const& layout, ScoreLocation const& location)
{
  ...
  // the line spacing.
  const int pixel_size = (layout.getStringCount() - 1) * layout.getTabLineSpacing() * 0.6;
  QFont font = MusicFont::getFont(pixel_size);

  auto clef = new SimpleTextItem(QChar(MusicFont::TabClef), font);

  auto pubsub = myScoreArea->getClickPubSub();
  auto group = new ClickableGroup(QObject::tr("Click to edit the number of strings."),
                                  ...
                                  bool isFirstStaff)
{
  for (Barline const& barline : system.getBarlines()) {
    ...
    double timeSigX = x + barlinePainter->boundingRect().width() + layout->getWidth(keySig);
    ...
    {
      x = LayoutInfo::STAFF_WIDTH - barlinePainter->boundingRect().width() / 2 - 1;
    ...
    if (keySig.isVisible()) {
      KeySignaturePainter* keySigPainter =
        new KeySignaturePainter(layout, keySig, location, myScoreArea->getClickPubSub());
    ...
    if (timeSig.isVisible()) {
      TimeSignaturePainter* timeSigPainter =
        new TimeSignaturePainter(layout, timeSig, location, myScoreArea->getClickPubSub());
    ...
    if (barline.hasRehearsalSign() && isFirstStaff) {
      ...
      Q_ASSERT(nextBar);
      const double signTextX = signLetters->x() + signLetters->boundingRect().width() + 7;
      // If the description is too wide, cut it off with an ellipsis.
      QString shortenedSignText =
        metrics.elidedText(QString::fromStdString(sign.getDescription()),
                           Qt::ElideRight,
                           layout->getPositionX(nextBar->getPosition()) - signTextX - RECTANGLE_OFFSET);

      auto signText = new SimpleTextItem(shortenedSignText, myRehearsalSignFont);
    ...
    for (Position const& pos : voice.getPositions()) {
      ...
      for (Note const& note : pos.getNotes()) {
        ...
        auto tabNote = new SimpleTextItem(text,
                                          myPlainTextFont,
                                          QPen(note.hasProperty(Note::Tied) ? Qt::lightGray : Qt::black),
                                          QBrush(QColor(255, 255, 255)));

        centerHorizontally(*tabNote, location, location + layout->getPositionSpacing());
        tabNote->setY(layout->getTabLine(note.getString() + 1) - 0.6 * myPlainTextFont.pixelSize());
...
void SystemRenderer::drawArpeggio(Position const& position, double x, LayoutInfo const& layout)
{
  ...
  const double top = layout.getTabLine(position.getNotes().front().getString() + 1);
  const double bottom = layout.getTabLine(position.getNotes().back().getString() + 1);
  ...
  auto arpeggio = new SimpleTextItem(QString(numSymbols, arpeggioSymbol), myMusicNotationFont);
  ...
  const QChar arpeggioEnd =
    position.hasProperty(Position::ArpeggioUp) ? MusicFont::ArpeggioUp : MusicFont::ArpeggioDown;
...
void SystemRenderer::drawAlternateEndings(System const& system, LayoutInfo const& layout, double height)
{
  ...
  for (AlternateEnding const& ending : system.getAlternateEndings()) {
    ...
    vertLine->setLine(0, TOP_LINE_OFFSET, 0, LayoutInfo::SYSTEM_SYMBOL_SPACING - TOP_LINE_OFFSET);
    vertLine->setPos(location, height);
    vertLine->setParentItem(myParentSystem);

    // Draw the text indicating the repeat numbers.
    auto text =
      new SimpleTextItem(QString::fromStdString(boost::lexical_cast<std::string>(ending)), myPlainTextFont);
    ...
    if (endX == 0) {
      endX = layout.getPositionX(system.getNextBarline(ending.getPosition())->getPosition());
    }
...
void SystemRenderer::drawTempoMarkers(System const& system, LayoutInfo const& layout, double height)
{
  for (TempoMarker const& tempo : system.getTempoMarkers()) {
    ...
    QString text;
    if (tempo.getMarkerType() == TempoMarker::AlterationOfPace) {
      text = (tempo.getAlterationOfPace() == TempoMarker::Accelerando) ? "accel." : "rit.";
    } else {
      ...
      auto pixmap = new QGraphicsPixmapItem(
        image.scaled(fm.width(imageSpacing), NOTE_HEIGHT, Qt::IgnoreAspectRatio, Qt::SmoothTransformation));
      ...
      if (tempo.getTripletFeel() != TempoMarker::NoTripletFeel) {
        ...
        pixmap = new QGraphicsPixmapItem(
          image.scaled(fm.width(imageSpacing), 21, Qt::IgnoreAspectRatio, Qt::SmoothTransformation));
        pixmap->setX(fm.width(text));
        centerSymbolVertically(*pixmap, height);
        group->addToGroup(pixmap);
...
void SystemRenderer::drawChordText(System const& system, LayoutInfo const& layout, double height)
{
  for (ChordText const& chord : system.getChords()) {
    ...
    auto textItem = new SimpleTextItem(QString::fromStdString(text), myPlainTextFont);
    textItem->setX(x);
    centerSymbolVertically(*textItem, height);
  ...
  for (Voice const& voice : staff.getVoices()) {
    ...
    for (Position const& pos : voice.getPositions()) {
      ...
      for (Note const& note : pos.getNotes()) {
        ...
        if (note.hasProperty(Note::HammerOnFromNowhere) || note.hasProperty(Note::PullOffToNowhere)) {
          ...
          auto arc = new AntialiasedPathItem(path);
          arc->setPos(layout.getPositionX(position) + 2, layout.getTabLine(string) - 2);
          arc->setParentItem(myParentStaff);
...
void SystemRenderer::drawPlayerChanges(System const& system, int staffIndex, LayoutInfo const& layout)
{
  for (PlayerChange const& change : system.getPlayerChanges()) {
    ...
    text->setPos(layout.getPositionX(change.getPosition()),
                 layout.getBottomStdNotationLine() + LayoutInfo::STAFF_BORDER_SPACING +
    ...
    for (int string = 0; string < staff.getStringCount(); ++string) {
      for (Position const& pos : voice.getPositions()) {
        ...
        if (note->hasProperty(Note::ShiftSlide) || note->hasProperty(Note::LegatoSlide) ||
          ...
          if (note->hasProperty(Note::ShiftSlide) || note->hasProperty(Note::LegatoSlide)) {
            const Note* nextNote = VoiceUtils::getNextNote(voice, pos.getPosition(), string);
            slideUp = nextNote && nextNote->getFretNumber() > note->getFretNumber();
          }

          const Position* nextPos = VoiceUtils::getNextPosition(voice, pos.getPosition());

          drawSlide(layout,
                    string,
                    slideUp,
                    pos.getPosition(),
                    nextPos ? nextPos->getPosition() : pos.getPosition() + 1);
        }

        // Draw any slides going into the note.
        if (note->hasProperty(Note::SlideIntoFromAbove) || note->hasProperty(Note::SlideIntoFromBelow)) {
          const int position = pos.getPosition();
          drawSlide(layout, string, note->hasProperty(Note::SlideIntoFromBelow), position - 1, position);
        }
      }
    }
  }
                               ...
                               int position2) const
{
  ...
  slide->setPos(left + layout.getPositionSpacing() / 1.5 + 1, y + height / 2);
  ...
  for (SymbolGroup const& symbolGroup : layout.getTabStaffBelowSymbols()) {
    ...
    switch (symbolGroup.getSymbolType()) {
      ...
      case SymbolGroup::ArtificialHarmonic: {
        const Position* pos =
          ScoreUtils::findByPosition(symbolGroup.getVoice().getPositions(), symbolGroup.getLeftPosition());
        Q_ASSERT(pos);
...
QGraphicsItem* SystemRenderer::createArtificialHarmonicText(Position const& position)
{
  ...
  return createPlainTextSymbol(QString::fromStdString(boost::lexical_cast<std::string>(name)),
                               QFont::StyleNormal);
}
  ...
  for (SymbolGroup const& symbolGroup : layout.getTabStaffAboveSymbols()) {
    ...
    switch (symbolGroup.getSymbolType()) {
      ...
      case SymbolGroup::LetRing:
        renderedSymbol = createConnectedSymbolGroup("let ring", QFont::StyleItalic, width, layout);
        break;
      ...
      case SymbolGroup::PalmMuting:
        renderedSymbol = createConnectedSymbolGroup("P.M.", QFont::StyleNormal, width, layout);
        break;
      ...
      case SymbolGroup::NaturalHarmonic:
        renderedSymbol = createConnectedSymbolGroup("N.H.", QFont::StyleNormal, width, layout);
        break;
      case SymbolGroup::Dynamic: {
        const Dynamic* dynamic =
          ScoreUtils::findByPosition(staff.getDynamics(), symbolGroup.getLeftPosition());
        Q_ASSERT(dynamic);
      ...
      case SymbolGroup::ArtificialHarmonic:
        renderedSymbol = createConnectedSymbolGroup("A.H.", QFont::StyleNormal, width, layout);
        break;
#if 0
  ...
  for (SymbolGroup const& symbolGroup : layout.getStdNotationStaffAboveSymbols()) {
    ...
    switch (symbolGroup.getSymbolType()) {
      case SymbolGroup::Octave8va:
        renderedSymbol =
          createConnectedSymbolGroup("8va", QFont::StyleItalic, symbolGroup.getWidth(), layout);
        break;
      case SymbolGroup::Octave15ma:
        renderedSymbol =
          createConnectedSymbolGroup("15ma", QFont::StyleItalic, symbolGroup.getWidth(), layout);
        break;
      default:
  ...
  for (SymbolGroup const& symbolGroup : layout.getStdNotationStaffBelowSymbols()) {
    ...
    switch (symbolGroup.getSymbolType()) {
      case SymbolGroup::Octave8vb:
        renderedSymbol =
          createConnectedSymbolGroup("8vb", QFont::StyleItalic, symbolGroup.getWidth(), layout);
        break;
      case SymbolGroup::Octave15mb:
        renderedSymbol =
          createConnectedSymbolGroup("15mb", QFont::StyleItalic, symbolGroup.getWidth(), layout);
        break;
      default:
        // All symbol types should have been dealt with by now.
        Q_ASSERT(false);
    ...
    renderedSymbol->setPos(layout.getPositionX(symbolGroup.getLeftPosition()),
                           layout.getBottomStdNotationLine() + layout.getStdNotationStaffBelowSpacing());
    renderedSymbol->setParentItem(myParentStaff);
  }
}
...
void SystemRenderer::createDashedLine(QGraphicsItemGroup* group, double left, double right, double y)
{
  ...
  auto lineEnd = new QGraphicsLineItem(
    line->boundingRect().right(), y, line->boundingRect().right(), y + 0.5 * LayoutInfo::TAB_SYMBOL_SPACING);
...
QGraphicsItem* SystemRenderer::createTremoloPicking(LayoutInfo const& layout)
{
  ...
  for (int i = 0; i < 3; i++) {
    auto line = new SimpleTextItem(QChar(MusicFont::TremoloPicking), myMusicNotationFont);
...
void SystemRenderer::drawStdNotation(System const& system, Staff const& staff, LayoutInfo const& layout)
{
  ...
  for (Voice const& voice : staff.getVoices()) {
    for (Position const& pos : voice.getPositions()) {
      ...
      if (pos.hasMultiBarRest()) {
        drawMultiBarRest(system, *prevBar, layout, pos.getMultiBarRestCount());
      } else if (pos.isRest()) {
  ...
  for (StdNotationNote const& note : notes) {
    ...
    const double x = layout.getPositionX(note.getPosition()) +
                     0.5 * (layout.getPositionSpacing() - note_head_width) - accidental_width;
    const double y = note.getY() + layout.getTopStdNotationLine() - fm->ascent();
    ...
    minNoteLocations[position] = std::min(minNoteLocations[position], note.getY());
    maxNoteLocations[position] = std::max(maxNoteLocations[position], note.getY());
                              ...
                              LayoutInfo const& layout)
{
  ...
  for (StdNotationNote const& note : notes) {
    ...
    const double x = layout.getPositionX(note.getPosition()) + 0.5 * layout.getPositionSpacing();
    const double prevX = layout.getPositionX(prevPos) + 0.5 * layout.getPositionSpacing();
    const double width = x - prevX;

    // Adjust the orientation depending on the stem direction.
    QPainterPath path;
    path.moveTo(width, height / 2);
    path.arcTo(0, 0, width, height, 0, stem.getStemType() == NoteStem::StemDown ? 180 : -180);

    const double y =
      note.getY() + layout.getTopStdNotationLine() +
      (stem.getStemType() == NoteStem::StemDown ? -1.25 * LayoutInfo::STD_NOTATION_LINE_SPACING
...
void SystemRenderer::drawIrregularGroups(Voice const& voice, const std::vector<NoteStem>& stems)
{
  ...
  for (IrregularGrouping const& group : voice.getIrregularGroupings()) {
    ...
    if (direction == NoteStem::StemUp) {
      ...
      y2 = highestStem.getTop() - topLayout.addBox(begin->getPositionIndex(), last->getPositionIndex(), 1) *
      ...
      y2 =
        lowestStem.getBottom() + bottomLayout.addBox(begin->getPositionIndex(), last->getPositionIndex(), 1) *
                                   LayoutInfo::IRREGULAR_GROUP_HEIGHT;
    }
    ...
    const QString text = QString::fromStdString(boost::lexical_cast<std::string>(group));
                                      ...
                                      int measureCount)
{
  ...
  const double leftX = (leftBar.getPosition() == 0) ? layout.getPositionX(leftBar.getPosition())
                                                    : layout.getPositionX(leftBar.getPosition() + 1);

  const double rightX = boost::algorithm::clamp(layout.getPositionX(rightBar->getPosition()),
                                                0.0,
                                                LayoutInfo::STAFF_WIDTH - layout.getPositionSpacing() / 2.0);

  // Draw the measure count.
  auto measureCountText = new SimpleTextItem(QString::number(measureCount), myMusicNotationFont);

  centerHorizontally(*measureCountText, leftX, rightX);
  measureCountText->setY(layout.getTopStdNotationLine() - myMusicFontMetrics.ascent());
  measureCountText->setParentItem(myParentStaff);

  // Draw symbol across std. notation staff.
  auto vertLineLeft =
    new QGraphicsLineItem(leftX, layout.getStdNotationLine(2), leftX, layout.getStdNotationLine(4));
  vertLineLeft->setParentItem(myParentStaff);

  auto vertLineRight =
    new QGraphicsLineItem(rightX, layout.getStdNotationLine(2), rightX, layout.getStdNotationLine(4));
  vertLineRight->setParentItem(myParentStaff);

  auto horizontalLine =
    new QGraphicsRectItem(leftX,
                          layout.getStdNotationLine(2) + 0.5 * LayoutInfo::STD_NOTATION_LINE_SPACING,
                          rightX - leftX,
                          LayoutInfo::STD_NOTATION_LINE_SPACING * 0.9);
...
void SystemRenderer::drawRest(Position const& pos, double x, LayoutInfo const& layout)
{
  ...
  const double dotY = 1.6 * LayoutInfo::STD_NOTATION_LINE_SPACING - myMusicFontMetrics.ascent();
                                     ...
                                     const std::map<int, double>& noteHeadWidths)
{
  ...
  for (const int position : minNoteLocations | boost::adaptors::map_keys) {
    ...
    const int numLedgerLinesTop = -highestNote / LayoutInfo::STD_NOTATION_LINE_SPACING;
    const int numLedgerLinesBottom =
      (lowestNote - layout.getStdNotationStaffHeight()) / LayoutInfo::STD_NOTATION_LINE_SPACING;
    ...
    if (numLedgerLinesTop > 0) {
      ...
      for (int i = 1; i <= numLedgerLinesTop; ++i) {
        ledgerLineLocations.push_back(topLine - i * LayoutInfo::STD_NOTATION_LINE_SPACING);
      }
    }
    ...
    if (numLedgerLinesBottom > 0) {
      ...
      for (int i = 1; i <= numLedgerLinesBottom; ++i) {
        ledgerLineLocations.push_back(bottomLine + i * LayoutInfo::STD_NOTATION_LINE_SPACING);
      }
    }

    const double ledgerLineWidth = noteHeadWidths.at(position) * 2;
    const double x = layout.getPositionX(position) + 0.5 * (layout.getPositionSpacing() - ledgerLineWidth);
...
void SystemRenderer::createBend(QGraphicsItemGroup* group,
                                double left,
                                double right,
                                double yStart,
                                double yEnd,
                                ...
                                bool prebend)
{
  ...
  arrowShape << QPointF(right - ARROW_WIDTH / 2, yEnd) << QPointF(right + ARROW_WIDTH / 2, yEnd)
             << QPointF(right, (yEnd < yStart) ? yEnd - ARROW_WIDTH : yEnd + ARROW_WIDTH);
  ...
  if (pitch != 0) {
    ...
    auto bendText = new SimpleTextItem(QString::fromStdString(Bend::getPitchText(pitch)), mySymbolTextFont);
...
QGraphicsItem* SystemRenderer::createBendGroup(SymbolGroup const& group, LayoutInfo const& layout)
{
  ...
  for (int i = group.getLeftPosition(); i < group.getRightPosition(); ++i) {
    const Position* pos = ScoreUtils::findByPosition(group.getVoice().getPositions(), i);
    if (!pos)
      continue;
    ...
    for (Note const& note : pos->getNotes()) {
      ...
      if (duration > 0) {
        ...
        for (int j = 0; j < duration && nextPos; ++j) {
          nextPos = VoiceUtils::getNextPosition(group.getVoice(), nextPos->getPosition());
        }
      ...
      if (type == Bend::ImmediateRelease)
        ...
        auto line = new QGraphicsLineItem(prevX, yStart, x + layout.getPositionSpacing(), yStart);
        line->setPen(QPen(Qt::black, 1, Qt::DashLine));
        itemGroup->addToGroup(line);

        // Draw the bend down to the new pitch.
        createBend(itemGroup,
                   x + layout.getPositionSpacing(),
                   rightX,
                   yStart,
                   yRelease,
                   bend.getReleasePitch(),
                   false);
      } else if (type == Bend::NormalBend || type == Bend::BendAndHold || type == Bend::PreBend ||
                 type == Bend::PreBendAndHold) {
        createBend(itemGroup,
                   leftX,
                   rightX,
                   yStart,
      ...
      } else if (type == Bend::BendAndRelease || type == Bend::PreBendAndRelease) {
        const double middleX =
          (type == Bend::BendAndRelease) ? 0.5 * (leftX + rightX) : x + layout.getPositionSpacing();

        // Figure out where to draw the first part of the bend.
        Bend::DrawPoint drawPoint = Bend::MidPoint;
        if (bend.getStartPoint() == Bend::MidPoint || bend.getEndPoint() == Bend::MidPoint) {
          drawPoint = Bend::HighPoint;
        }

        const double yMiddle = getBendHeight(drawPoint, note, layout);
        createBend(
          itemGroup, leftX, middleX, yStart, yMiddle, bend.getBentPitch(), type == Bend::PreBendAndRelease);

        // Draw the second part of the bend.
        createBend(itemGroup, middleX, rightX, yMiddle, yRelease, bend.getReleasePitch(), false);
      }
