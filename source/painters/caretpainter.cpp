...
void CaretPainter::paint(QPainter* painter, const QStyleOptionGraphicsItem*, QWidget*)
{
  if (!myLayout) {
    onLocationChanged();
    return;
  ...
  const double x = LayoutInfo::centerItem(left, left + myLayout->getPositionSpacing(), 1);
  ...
  const double stringHeight = myLayout->getTabLine(location.getString() + 1) - myLayout->getTopTabLine();
  ...
  const double width = std::min(myLayout->getPositionSpacing(), LayoutInfo::DEFAULT_POSITION_SPACING);

  left = LayoutInfo::centerItem(left, left + myLayout->getPositionSpacing(), width);
  ...
  left = myLayout->getPositionX(std::min(location.getSelectionStart(), location.getPositionIndex()));
  double right = myLayout->getPositionX(std::max(location.getSelectionStart(), location.getPositionIndex()));

  if (left != right)
    right += myLayout->getPositionSpacing();

  painter->drawRect(QRectF(left, 1, right - left, myLayout->getTabStaffHeight() - 1));
}
...
void CaretPainter::onLocationChanged()
{
  ...
  myLayout.reset(new LayoutInfo(
    location.getScore(), system, location.getSystemIndex(), location.getStaff(), location.getStaffIndex()));

  const ViewFilter* filter =
    myViewOptions.getFilter() ? &location.getScore().getViewFilters()[*myViewOptions.getFilter()] : nullptr;

  // Compute the offset due to the previous (visible) staves.
  double offset = 0;
  for (int i = 0; i < location.getStaffIndex(); ++i) {
    if (!filter || filter->accept(location.getScore(), location.getSystemIndex(), i)) {
      offset += LayoutInfo(location.getScore(), system, location.getSystemIndex(), system.getStaves()[i], i)
                  .getStaffHeight();
    }
  }
  ...
  setPos(0,
         mySystemRects.at(location.getSystemIndex()).top() + offset + myLayout->getSystemSymbolSpacing() +
           myLayout->getStaffHeight() - myLayout->getTabStaffBelowSpacing() - myLayout->STAFF_BORDER_SPACING -
           myLayout->getTabStaffHeight());
  update(boundingRect());
  // Ensure that a redraw always occurs at the old location.
  scene()->update(oldRect);

  setToolTip(QString::fromStdString(boost::lexical_cast<std::string>(location)));
