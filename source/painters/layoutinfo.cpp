  ...
  , myStdNotationStaffBelowSpacing(0)
{
  ...
  StdNotationNote::getNotesInStaff(
    score, system, systemIndex, staff, staffIndex, *this, myNotes, myStems, myBeamGroups);
...
{
  return myStdNotationStaffAboveSpacing + myStdNotationStaffBelowSpacing + myTabStaffAboveSpacing +
         myTabStaffBelowSpacing + STD_NOTATION_LINE_SPACING * (NUM_STD_NOTATION_LINES - 1) +
         (getStringCount() - 1) * getTabLineSpacing() + 4 * STAFF_BORDER_SPACING;
}

double LayoutInfo::getStdNotationLine(int line) const
{
  return myStdNotationStaffAboveSpacing + STAFF_BORDER_SPACING + (line - 1) * STD_NOTATION_LINE_SPACING;
}
...
double LayoutInfo::getFirstPositionX() const
{
  ...
  const double barlineWidth = (startBar.getBarType() == Barline::SingleBar) ? 1 : getPositionSpacing();
  width += barlineWidth;
...
double LayoutInfo::getWidth(const Barline& bar)
{
  ...
  if (width > 0) {
    ...
    if (bar.getBarType() == Barline::DoubleBar)
      ...
      width += 5;
    else if (bar.getBarType() == Barline::RepeatEnd || bar.getBarType() == Barline::DoubleBarFine)
      width += 6;
  }
...
void LayoutInfo::calculateTabStaffBelowLayout()
{
  for (const Voice& voice : myStaff.getVoices()) {
    for (const Position& pos : voice.getPositions()) {
      ...
      const int rightPosition = nextPos ? nextPos->getPosition() : leftPosition + 1;
      const double width = getPositionSpacing();

      if (pos.hasProperty(Position::PickStrokeUp)) {
        myTabStaffBelowSymbols.emplace_back(
          SymbolGroup::PickStrokeUp, leftPosition, rightPosition, voice, width, height++);
      }

      if (pos.hasProperty(Position::PickStrokeDown)) {
        myTabStaffBelowSymbols.emplace_back(
          SymbolGroup::PickStrokeDown, leftPosition, rightPosition, voice, width, height++);
      }

      if (pos.hasProperty(Position::Tap) || Utils::hasNoteWithTappedHarmonic(pos)) {
        myTabStaffBelowSymbols.emplace_back(
          SymbolGroup::Tap, leftPosition, rightPosition, voice, width, height++);
      }

      if (Utils::hasNoteWithArtificialHarmonic(pos)) {
        myTabStaffBelowSymbols.emplace_back(
          SymbolGroup::ArtificialHarmonic, leftPosition, rightPosition, voice, width, height++);
      }

      if (VoiceUtils::hasNoteWithHammerOn(voice, pos) ||
          Utils::hasNoteWithProperty(pos, Note::HammerOnFromNowhere)) {
        myTabStaffBelowSymbols.emplace_back(
          SymbolGroup::Hammeron, leftPosition, rightPosition, voice, width, height++);
        ...
        myTabStaffBelowSymbols.emplace_back(
          SymbolGroup::Pulloff, leftPosition, rightPosition, voice, width, height++);
      }
      ...
      if (Utils::hasNoteWithProperty(pos, Note::SlideIntoFromAbove) ||
        ...
        myTabStaffBelowSymbols.emplace_back(
          SymbolGroup::Slide, leftPosition, rightPosition, voice, width, height++);
      }
    }
  }

  // Compute the overall spacing needed below the tab staff.
  myTabStaffBelowSpacing = getMaxHeight(myTabStaffBelowSymbols) * TAB_SYMBOL_SPACING;
}
...
void LayoutInfo::calculateStdNotationStaffAboveLayout()
{
  ...
  myStdNotationStaffAboveSpacing = getMaxHeight(myStdNotationStaffAboveSymbols) * TAB_SYMBOL_SPACING;
...
void LayoutInfo::calculateStdNotationStaffBelowLayout()
{
  ...
  myStdNotationStaffBelowSpacing = getMaxHeight(myStdNotationStaffBelowSymbols) * TAB_SYMBOL_SPACING;
  ...
  myStdNotationStaffBelowSpacing += std::max(bottomBoundary, maxLocation) - bottomBoundary;
}
    ...
    for (const Position& pos : voice.getPositions()) {
      ...
      if (type != currentType || endOfStaff) {
        if (currentType != SymbolGroup::NoSymbol) {
          ...
          symbols.emplace_back(currentType, leftPos, rightPos, voice, right - left, 1);
        }
...
void LayoutInfo::calculateTabStaffAboveLayout()
{
  ...
  for (int symbolIndex = SymbolGroup::LetRing; symbolIndex <= SymbolGroup::ArtificialHarmonic;
       ++symbolIndex) {
    SymbolGroup::SymbolType symbol = static_cast<SymbolGroup::SymbolType>(symbolIndex);
    bool inGroup = false;
    int leftPos = 0;
    int rightPos = 0;
    ...
    for (int i = 0; i < static_cast<int>(symbolSets.size()); ++i) {
      ...
      if ((!hasSymbol && inGroup) ||
          (hasSymbol && (symbol == SymbolGroup::Trill || symbol == SymbolGroup::TremoloPicking ||
        ...
        myTabStaffAboveSymbols.emplace_back(
          symbol, leftPos, rightPos, myStaff.getVoices()[0], rightX - leftX, y);
      }
      // Start a new group.
...
void LayoutInfo::calculateBendLayout(VerticalLayout& layout)
{
  for (const Voice& voice : myStaff.getVoices()) {
    ...
    for (unsigned int i = 0; i < voice.getPositions().size(); ++i) {
      ...
      if (type == Bend::NormalBend || type == Bend::BendAndRelease || type == Bend::PreBend ||
          type == Bend::PreBendAndRelease || type == Bend::GradualRelease || type == Bend::ImmediateRelease) {
        if (!inGroup)
          leftPos = pos.getPosition();
        rightPos++;

        const int y = layout.addBox(leftPos, rightPos, groupHeight);
        myTabStaffAboveSymbols.emplace_back(SymbolGroup::Bend, leftPos, rightPos, voice, 0, y);
        inGroup = false;
        groupHeight = 0;
        rightPos = 0;
      } else if (!inGroup) {
