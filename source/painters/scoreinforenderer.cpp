...
static const QString theAudioReleaseTypes[] = { "Single",       "EP",           "Album",
                                                "Double Album", "Triple Album", "Boxset" };
...
static void addCenteredText(QGraphicsItemGroup& group, QFont font, int font_size, const QString& text)
{
  ...
  // Center horizontally.
  text_item->setX(LayoutInfo::centerItem(0.0, LayoutInfo::STAFF_WIDTH, text_item->boundingRect().width()));
...
static void renderReleaseInfo(QGraphicsItemGroup& group, const QFont& font, const SongData& song_data)
{
  ...
  if (song_data.isAudioRelease()) {
    const SongData::AudioReleaseInfo& audio_release = song_data.getAudioReleaseInfo();
    if (audio_release.getTitle().empty())
      return;

    const int release_type = static_cast<int>(audio_release.getReleaseType());

    release_info = QString("(From the %1 %2 \"%3\")");
    release_info = release_info.arg(QString::number(audio_release.getYear()),
                                    theAudioReleaseTypes[release_type],
                                    QString::fromStdString(audio_release.getTitle()));
  } else if (song_data.isVideoRelease()) {
    const SongData::VideoReleaseInfo& video_release = song_data.getVideoReleaseInfo();
    if (video_release.getTitle().empty())
      return;

    // TODO - handle live video / audio releases?
    release_info = QString("(From the Video \"%1\")").arg(QString::fromStdString(video_release.getTitle()));
  } else if (song_data.isBootleg()) {
    const SongData::BootlegInfo& bootleg = song_data.getBootlegInfo();
    if (bootleg.getTitle().empty())
      return;

    QDate date(bootleg.getDate().year(), bootleg.getDate().month(), bootleg.getDate().day());

    release_info = QString("(From the %1 Bootleg \"%2\")")
                     .arg(date.toString("MMM d yyyy"), QString::fromStdString(bootleg.getTitle()));
  } else
    return;
                          ...
                          bool right_align = false)
{
  ...
  const double text_width = std::min(AUTHOR_INFO_WIDTH, text_item->boundingRect().width());
...
static void renderAuthorInfo(QGraphicsItemGroup& group, QFont font, const SongData& song_data)
{
  ...
  if (!song_data.getTranscriber().empty()) {
    ...
    transcriber_text = transcriber_text.arg(QString::fromStdString(song_data.getTranscriber()));
...
static void renderSongInfo(QGraphicsItemGroup& group, const QFont& font, const SongData& song_data)
{
  ...
  if (!song_data.getArtist().empty()) {
    ...
    artist_info = artist_info.arg(QString::fromStdString(song_data.getArtist()));
...
static void renderLessonInfo(QGraphicsItemGroup& group, const QFont& font, const LessonData& lesson_data)
{
  ...
  if (!lesson_data.getSubtitle().empty()) {
    addCenteredText(group, font, SUBTITLE_SIZE, QString::fromStdString(lesson_data.getSubtitle()));
  }
  ...
  if (!lesson_data.getAuthor().empty()) {
    ...
    author_text = author_text.arg(QString::fromStdString(lesson_data.getAuthor()));
