...
KeySignaturePainter::KeySignaturePainter(LayoutConstPtr const& layout,
  ...
  , myBounds(0, -10, LayoutInfo::getWidth(myKeySignature), layout->getStdNotationStaffHeight())
{
...
void KeySignaturePainter::drawAccidentals(QVector<double>& positions, QChar accidental, QPainter* painter)
{
  ...
  for (int i = 0; i < myKeySignature.getNumAccidentals(true); ++i) {
    painter->drawText(i * LayoutInfo::ACCIDENTAL_WIDTH, positions.at(i), accidental);
  }
}
