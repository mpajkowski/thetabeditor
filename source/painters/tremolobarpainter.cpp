...
TremoloBarPainter::TremoloBarPainter(const Position *position, double width)
    : position(position), width(width), textFont("Liberation Sans")
{
...
void TremoloBarPainter::paint(QPainter *painter,
                              const QStyleOptionGraphicsItem *, QWidget *)
{
    ...
    {
        displayWidth /=
            2.0; // don't want to stretch a dip into the next position
    }

    const QString displayText =
        QString::fromStdString(position->GetTremoloBarText());
    ...
    if (type != Position::release)
    {
        ...
        painter->drawText(
            centerItem(0, displayWidth,
                       QFontMetricsF(textFont).width(displayText)),
