...
static const std::unordered_map<char, int> theNotePositions = { { 'F', 0 }, { 'E', 1 },  { 'D', 2 },
                                                                { 'C', 3 }, { 'B', -3 }, { 'A', -2 },
                                                                { 'G', -1 } };
  ...
  , myTie(tie)
{
  ...
  if (note.hasProperty(Note::NaturalHarmonic) || note.hasTappedHarmonic() || note.hasArtificialHarmonic()) {
    myNoteHeadSymbol = (pos.getDurationType() <= Position::HalfNote) ? MusicFont::HarmonicNoteHeadOpen
    ...
    for (Barline const& bar : system.getBarlines()) {
      ...
      for (Position const& pos :
        ...
        if (pos.isRest() || pos.hasMultiBarRest()) {
          const double x = layout.getPositionX(pos.getPosition()) + 0.5 * layout.getPositionSpacing();
        ...
        for (Note const& note : pos.getNotes()) {
          ...
          notes.push_back(StdNotationNote(voice, pos, note, bar.getKeySignature(), tuning, y, tiedPos));
          ...
          if (accidentals.find(y) != accidentals.end() &&
            ...
            if (accidentals.find(y) != accidentals.end() && accidental == NoAccidental) {
          ...
          const QFontMetricsF* fm = stdNote.isGraceNote() ? &grace_fm : &default_fm;
          noteHeadWidth = fm->width(stdNote.getNoteHeadSymbol());
        }

        const double x =
          layout.getPositionX(pos.getPosition()) + 0.5 * (layout.getPositionSpacing() - noteHeadWidth);
        stems.push_back(NoteStem(voice, pos, x, noteHeadWidth, noteLocations));
      }
                                        ...
                                        Tuning const& tuning)
{
  ...
  const std::string text = Midi::getMidiNoteText(pitch,
                                                 key.getKeyType() == KeySignature::Minor,
  ...
  const int topNote = (staff.getClefType() == Staff::TrebleClef) ? Midi::MIDI_NOTE_F4 : Midi::MIDI_NOTE_A2;

  // Shift the note by the number of octaves.
  y += 7 * (Midi::getMidiNoteOctave(topNote) - Midi::getMidiNoteOctave(pitch, text[0])) +
...
void StdNotationNote::computeAccidentalType(bool explicitSymbol)
{
  ...
  const int pitch = myTuning->getNote(myNote->getString(), true) + myNote->getFretNumber();
  const bool usesSharps = myKey->usesSharps() || myKey->getNumAccidentals() == 0;

  const std::string noteText = Midi::getMidiNoteText(pitch,
                                                     myKey->getKeyType() == KeySignature::Minor,
                                                     usesSharps,
                                                     myKey->getNumAccidentals(),
                                                     explicitSymbol);
...
std::vector<uint8_t> StdNotationNote::getBeamingPatterns(TimeSignature const& timeSig)
{
  ...
  beaming.erase(std::remove(beaming.begin(), beaming.end(), 0), beaming.end());
  return beaming;
}
                                     ...
                                     std::vector<BeamGroup>& groups)
{
  ...
  std::transform(stems.begin() + firstStemIndex,
                 stems.end(),
                 durations.begin(),
  ...
  while (groupEnd != durations.end()) {
    ...
    // be.
    const double groupEndTime = *groupSize * std::min(0.5, 4.0 / timeSig.getBeatValue()) + groupBeginTime;
  ...
  while (i != lastStemIndex) {
    ...
    while (i < lastStemIndex && !NoteStem::isBeamable(stems[i])) {
      ...
      if (NoteStem::needsStem(stems[i])) {
        groups.push_back(BeamGroup(NoteStem::formatGroup(stems, { i }), { i }));
      }
    ...
    while (i < lastStemIndex && NoteStem::isBeamable(stems[i])) {
      ...
      if (stems[i].isGraceNote()) {
        ...
        if (!group_stems.empty()) {
          if (subgroupLength) {
            if (std::abs(std::fmod(durations[i - firstStemIndexInBar - 1], *subgroupLength)) >= 0.001) {
