...
static int getNumExtraBeams(const NoteStem& stem)
{
  ...
  // 3 = 1).
  return std::log(static_cast<double>(stem.getDurationType())) / std::log(2.0) - 3;
}

BeamGroup::BeamGroup(NoteStem::StemType direction, const std::vector<size_t>& stems)
  : myStemDirection(direction)
  , myStems(stems)
{}
                               ...
                               std::vector<NoteStem>::const_iterator end) const
{
  for (auto stem = begin; stem != end; ++stem) {
    ...
    const Position::DurationType prevDuration =
      (stem != begin) ? prevStem->getDurationType() : Position::SixtyFourthNote;
    const Position::DurationType nextDuration =
      (nextStem != end) ? nextStem->getDurationType() : Position::SixtyFourthNote;
    ...
    const bool hasFractionalLeft =
      stem != begin &&
      ((duration > prevDuration && (nextStem == end || nextDuration == Position::EighthNote)) ||
       nextStem == end || !nextStem->hasFullBeaming());
    const bool hasFractionalRight =
      !hasFractionalLeft && (duration > nextDuration || (nextStem != end && !stem->hasFullBeaming()));
                                         ...
                                         const QFontMetricsF& fm)
{
  ...
  const double xPos = (stem.getStemType() == NoteStem::StemUp) ? stem.getX() - HORIZONTAL_OFFSET
                                        ...
                                        const LayoutInfo& layout)
{
  ...
  const QChar symbol =
    (stem.getStemType() == NoteStem::StemUp) ? MusicFont::FermataUp : MusicFont::FermataDown;
                                         ...
                                         const QFontMetricsF& fm)
{
  ...
  if (stem.isGraceNote()) {
    ...
    const QChar slash_symbol =
      stem.getStemType() == NoteStem::StemUp ? MusicFont::GraceNoteSlashUp : MusicFont::GraceNoteSlashDown;
