...
TimeSignaturePainter::TimeSignaturePainter(LayoutConstPtr const& layout,
  ...
  , myLocation(location)
  , myPubSub(pubsub)
  , myBounds(0, 0, LayoutInfo::getWidth(myTimeSignature), myLayout->getStdNotationStaffHeight())
{
...
void TimeSignaturePainter::paint(QPainter* painter, const QStyleOptionGraphicsItem*, QWidget*)
{
  const TimeSignature::MeterType meterType = myTimeSignature.getMeterType();
  if (meterType == TimeSignature::CommonTime || meterType == TimeSignature::CutTime) {
    painter->setFont(MusicFont::getFont(25));

    const QChar symbol =
      (meterType == TimeSignature::CommonTime) ? MusicFont::CommonTime : MusicFont::CutTime;
    painter->drawText(0, 2 * LayoutInfo::STD_NOTATION_LINE_SPACING, symbol);
  } else {
    drawNumber(painter, 2 * LayoutInfo::STD_NOTATION_LINE_SPACING, myTimeSignature.getBeatsPerMeasure());
    drawNumber(painter, 4 * LayoutInfo::STD_NOTATION_LINE_SPACING, myTimeSignature.getBeatValue());
  }
}
...
void TimeSignaturePainter::drawNumber(QPainter* painter, const double y, const int number) const
{
  QString text = QString::number(number);
  QFont font = MusicFont::getFont(27);

  const double width = QFontMetricsF(font).width(text);
  const double x = LayoutInfo::centerItem(0, LayoutInfo::getWidth(myTimeSignature), width);
