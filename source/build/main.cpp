...
static void displayError(const std::string& reason)
{
  ...
  if (!QApplication::instance())
    ...
    CrashDialog dialog(QString::fromStdString(message), QApplication::activeWindow());
    dialog.exec();
  }
  ...
  virtual bool event(QEvent* event) override
  {
    switch (event->type()) {
      ...
      case QEvent::FileOpen: {
        auto app = dynamic_cast<TheTabEditor*>(activeWindow());
        Q_ASSERT(app);

        app->openFiles({ static_cast<QFileOpenEvent*>(event)->file() });
        return true;
      }
      default:
...
int main(int argc, char* argv[])
{
  ...
  po::options_description desc("Usage: thetabeditor [options] [files...] "
                               "\nA guitar tablature editor.\n\nOptions");
  try {
    desc.add_options()("help,h", "Displays this help.")("version,v", "Displays version information.")(
      "files", po::value<std::vector<std::string>>(), "The files to be opened, optionally.");
