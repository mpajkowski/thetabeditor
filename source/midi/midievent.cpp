...
{
  return MidiEvent(ticks, { StatusByte::MetaMessage, MetaType::TrackEnd, 0 }, SystemLocation(), -1, -1);
}

bool MidiEvent::isTempoChange() const
{
  return getStatusByte() == StatusByte::MetaMessage && myData[1] == MetaType::SetTempo;
}
...
MidiEvent MidiEvent::setTempo(int ticks, int microseconds)
{
  ...
  return MidiEvent(ticks,
                   { StatusByte::MetaMessage,
                     static_cast<uint8_t>(MetaType::SetTempo),
                     3,
...
MidiEvent MidiEvent::noteOn(int ticks,
                            uint8_t channel,
                            uint8_t pitch,
                            uint8_t velocity,
                            const SystemLocation& location)
...
MidiEvent MidiEvent::volumeChange(int ticks, uint8_t channel, uint8_t level)
{
  return MidiEvent(
    ticks,
    { static_cast<uint8_t>(StatusByte::ControlChange + channel), Controller::ChannelVolume, level },
    SystemLocation(),
    -1,
    -1);
}
...
{
  return MidiEvent(
    ticks, { static_cast<uint8_t>(StatusByte::ProgramChange + channel), preset }, SystemLocation(), -1, -1);
}

MidiEvent MidiEvent::modWheel(int ticks, uint8_t channel, uint8_t width)
{
  return MidiEvent(ticks,
                   { static_cast<uint8_t>(StatusByte::ControlChange + channel), Controller::ModWheel, width },
                   SystemLocation(),
                   -1,
                   -1);
}
...
MidiEvent MidiEvent::holdPedal(int ticks, uint8_t channel, bool enabled)
{
  return MidiEvent(ticks,
                   { static_cast<uint8_t>(StatusByte::ControlChange + channel),
                     Controller::HoldPedal,
                     static_cast<uint8_t>(enabled ? 127 : 0) },
                   SystemLocation(),
                   -1,
                   -1);
}
...
{
  return getStatusByte() == StatusByte::SysEx && myData[1] == theSysExManufacturerId;
}
...
std::vector<MidiEvent> MidiEvent::pitchWheelRange(int ticks, uint8_t channel, uint8_t semitones)
{
  return {
    MidiEvent(ticks,
              { static_cast<uint8_t>(StatusByte::ControlChange + channel), Controller::RpnMsb, 0 },
              SystemLocation(),
              -1,
              -1),
    MidiEvent(ticks,
              { static_cast<uint8_t>(StatusByte::ControlChange + channel), Controller::RpnLsb, 0 },
              SystemLocation(),
              -1,
              -1),
    MidiEvent(
      ticks,
      { static_cast<uint8_t>(StatusByte::ControlChange + channel), Controller::DataEntryCoarse, semitones },
      SystemLocation(),
      -1,
      -1),
    MidiEvent(ticks,
              { static_cast<uint8_t>(StatusByte::ControlChange + channel), Controller::DataEntryFine, 0 },
              SystemLocation(),
              -1,
              -1),
  };
