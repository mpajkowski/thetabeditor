  ...
  while (!input.isAtEnd() && input.getLocation() < length) {
    ...
    // offset.
      ...
      const int32_t length =
        boost::algorithm::clamp<int32_t>(input.readBits(p, Gpx::BitStream::Reversed), 0, offset);

      std::copy(output.begin() + startPos, output.begin() + startPos + length, std::back_inserter(output));
    }
  }
...
const std::string& Gpx::FileSystem::getFileContents(const std::string& filename) const
{
    ...
    if (Util::readUInt(data, offset) == 2) {
      ...
      while ((block = Util::readUInt(data, blockIndex + 4 * blockCount)) != 0) {
        ...
        std::copy(data.begin() + offset,
                  data.begin() + boost::algorithm::clamp<size_t>(offset + SECTOR_SIZE, 0, data.size()),
      ...
      if (fileData.size() >= fileSize) {
        std::string fileName;
        std::copy(
          data.begin() + fileNameIndex, data.begin() + fileNameIndex + 127, std::back_inserter(fileName));
