...
void Gpx::DocumentReader::readHeader(Score& score)
{
  ...
  data.setAudioReleaseInfo(SongData::AudioReleaseInfo(SongData::AudioReleaseInfo::ReleaseType::Album,
                                                      header.child_value("Album"),
                                                      boost::gregorian::day_clock::local_day().year(),
                                                      false));

  data.setAuthorInfo(SongData::AuthorInfo(header.child_value("Music"), header.child_value("Words")));
...
void Gpx::DocumentReader::readTracks(Score& score)
{
  ...
  for (xml_node track : myFile.child("Tracks")) {
    ...
    player.setMidiPreset(track.child("GeneralMidi").child("Program").text().as_int());
    ...
    if (properties) {
      ...
      if (pitches) {
        ...
        tuning.setNotes(std::vector<uint8_t>(tuningNotes.rbegin(), tuningNotes.rend()));
      }
...
void Gpx::DocumentReader::readRhythms()
{
  for (xml_node currentRhythm : myFile.child("Rhythms")) {
    ...
    std::map<std::string, int> noteValuesToInt = { { "Whole", 1 },  { "Half", 2 },  { "Quarter", 4 },
...
{
  for (xpath_node node : myFile.select_nodes("./MasterTrack/Automations/Automation")) {
  ...
  for (xml_node masterBar : myFile.child("MasterBars")) {
    ...
    if (myAutomations.find(barIndex) != myAutomations.end()) {
      ...
      if (automation.type == "Tempo") {
        ...
        marker.setBeatsPerMinute(automation.value[0] * static_cast<int>(automation.value[1] / 2.0));
    ...
    for (unsigned int i = 0; i < score.getPlayers().size() && i < static_cast<unsigned int>(barIds.size());
         ++i) {
      Staff& staff = system.getStaves()[i];
      int currentPos = (startPos != 0) ? startPos + 1 : 0;

      // TODO - import multiple voices.
      for (int beatId : myVoices[myBars[barIds[i]].voiceIds.at(0)].beatIds) {
        ...
        pos.setDurationType(static_cast<Position::DurationType>(rhythm.noteValue));
        ...
        for (int noteId : beat.noteIds) {
          Note note = convertNote(noteId, pos, score.getPlayers()[i].getTuning());
...
Note Gpx::DocumentReader::convertNote(int noteId, Position& position, Tuning const& tuning) const
{
  ...
  if (gpxNote.trillNote != -1) {
    ptbNote.setTrilledFret(gpxNote.trillNote - tuning.getNote(ptbNote.getString(), false));
  }
  ...
  for (xml_node property : gpxNote.properties) {
    ...
    if (propertyName == "String") {
      ptbNote.setString(tuning.getStringCount() - property.child("String").text().as_int() - 1);
