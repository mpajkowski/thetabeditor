...
{
  myInstrument = boost::algorithm::clamp<int32_t>(stream.read<int32_t>(), 0, Midi::NUM_MIDI_PRESETS);
...
void Note::load(InputStream& stream)
{
  ...
  if (stream.version <= Version4 && flags.test(NoteHeader::TimeIndependentDuration)) {
...
void Beat::loadBeatEffects(InputStream& stream)
{
  ...
  // GP3 effect decoding.
  if (stream.version == Version3) {
    myIsVibrato = flags1.test(BeatEffects::VibratoGp3_1) || flags1.test(BeatEffects::VibratoGp3_2);
    myIsNaturalHarmonic = flags1.test(BeatEffects::NaturalHarmonicGp3);
    myIsArtificialHarmonic = flags1.test(BeatEffects::ArtificialHarmonicGp3);
  } else
  ...
  if (stream.version >= Version4 && flags2.test(BeatEffects::HasTremoloBarEvent)) {
    loadTremoloBar(stream);
  }
...
void Measure::load(InputStream& stream)
{
  ...
  if (flags.test(MeasureHeader::Numerator) || flags.test(MeasureHeader::Denominator)) {
