...
void MidiPlayer::run()
{
  ...
  BOOST_SCOPE_EXIT(this_) { CoUninitialize(); }
  BOOST_SCOPE_EXIT_END
#endif

  boost::signals2::scoped_connection connection(mySettingsManager.subscribeToChanges([&]() {
  ...
  const SystemLocation start_location(myStartLocation.getSystemIndex(), myStartLocation.getPositionIndex());
  ...
  for (auto event = events.begin(); event != events.end(); ++event) {
    ...
    const int duration_us =
      boost::rational_cast<int>(boost::rational<int>(delta, ticks_per_beat) * beat_duration);

    usleep(duration_us * (100.0 / myPlaybackSpeed));

    // Don't play metronome events if the metronome is disabled.
    if (event->isNoteOnOff() && event->getChannel() == METRONOME_CHANNEL && !myMetronomeEnabled) {
      continue;
    }
...
void MidiPlayer::performCountIn(MidiOutputDevice& device, SystemLocation const& location, int beat_duration)
{
  ...
  uint8_t preset;
  {
    ...
    preset = settings->get(Settings::CountInPreset) + Midi::MIDI_PERCUSSION_PRESET_OFFSET;
  }
  ...
  const int tick_duration = boost::rational_cast<int>(
    ...
    boost::rational<int>(time_sig.getBeatsPerMeasure(), time_sig.getNumPulses()) * beat_duration);
