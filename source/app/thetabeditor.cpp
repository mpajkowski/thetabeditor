  ...
  , myPlaybackArea(nullptr)
{
  ...
  connect(myUndoManager.get(), SIGNAL(redrawNeeded(int)), this, SLOT(redrawSystem(int)));
  connect(myUndoManager.get(), SIGNAL(fullRedrawNeeded()), this, SLOT(redrawScore()));
  connect(myUndoManager.get(), SIGNAL(cleanChanged(bool)), this, SLOT(updateModified(bool)));
  ...
  myRecentFiles = new RecentFiles(*mySettingsManager, myRecentFilesMenu, this);
  connect(myRecentFiles, SIGNAL(fileSelected(QString)), this, SLOT(openFile(QString)));

  createTabArea();

  auto settings = mySettingsManager->getReadHandle();
  myPreviousDirectory = QString::fromStdString(settings->get(Settings::PreviousDirectory));
...
void TheTabEditor::createNewDocument()
{
  myDocumentManager->addDefaultDocument(*mySettingsManager);
  setupNewTab();
}

void TheTabEditor::openFile(QString filename)
{
  if (filename.isEmpty()) {
    filename = QFileDialog::getOpenFileName(
      this, tr("Open"), myPreviousDirectory, QString::fromStdString(myFileFormatManager->importFileFilter()));
  }
  ...
  boost::optional<FileFormat> format = myFileFormatManager->findFormat(fileInfo.suffix().toStdString());

  if (!format) {
    QMessageBox::warning(this, tr("Error Opening File"), tr("Unsupported file type."));
    return;
  }
  ...
  try {
    ...
    qDebug() << "File loaded in" << std::chrono::duration_cast<std::chrono::milliseconds>(end - start).count()
             << "ms";
    ...
    myDocumentManager->removeDocument(myDocumentManager->getCurrentDocumentIndex());

    QMessageBox::warning(this, tr("Error Opening File"), tr("Error opening file: %1").arg(QString(e.what())));
  }
}

void TheTabEditor::switchTab(int index)
{
...
bool TheTabEditor::closeTab(int index)
{
...
bool TheTabEditor::closeCurrentTab()
{
...
bool TheTabEditor::saveFile()
{
  ...
  return QFileInfo(filename).suffix() == "pt2" ? saveFile(filename) : saveFileAs();
}

bool TheTabEditor::saveFile(QString path)
{
  QFileInfo info(path);
  QString extension = info.suffix();
  Q_ASSERT(!extension.isEmpty());

  boost::optional<FileFormat> format = myFileFormatManager->findFormat(extension.toStdString());
  if (!format) {
    QMessageBox::warning(this, tr("Error Saving File"), tr("Unsupported file type."));
    return false;
  }
  ...
  try {
    ...
    QMessageBox::warning(this, tr("Error Saving File"), tr("Error saving file: %1").arg(QString(e.what())));
...
bool TheTabEditor::saveFileAs()
{
  const QString filter = QString::fromStdString(myFileFormatManager->exportFileFilter());
...
void TheTabEditor::updateModified(bool clean)
{
  setWindowModified(!clean);
}

void TheTabEditor::cycleTab(int offset)
{
...
void TheTabEditor::editKeyboardShortcuts()
{
...
void TheTabEditor::printPreview()
{
...
void TheTabEditor::cutSelectedNotes()
{
...
void TheTabEditor::copySelectedNotes()
{
  Clipboard::copySelection(getLocation());
}

void TheTabEditor::pasteNotes()
{
...
void TheTabEditor::polishScore()
{
  myUndoManager->push(new PolishScore(getLocation().getScore()), UndoManager::AFFECTS_ALL_SYSTEMS);
}

void TheTabEditor::polishSystem()
{
  myUndoManager->push(new PolishSystem(getLocation()), getLocation().getSystemIndex());
}

void TheTabEditor::editFileInformation()
{
  FileInformationDialog dialog(this, myDocumentManager->getCurrentDocument());
  if (dialog.exec() == QDialog::Accepted) {
    myUndoManager->push(new EditFileInformation(getLocation(), dialog.getScoreInfo()),
                        UndoManager::AFFECTS_ALL_SYSTEMS);
  }
}

void TheTabEditor::startStopPlayback(bool from_measure_start)
{
  ...
  if (myIsPlaying) {
    ...
    connect(myMidiPlayer.get(), SIGNAL(playbackPositionChanged(int)), this, SLOT(moveCaretToPosition(int)));
    connect(myMidiPlayer.get(), SIGNAL(finished()), this, SLOT(startStopPlayback()));
    connect(myPlaybackWidget,
...
void TheTabEditor::redrawSystem(int index)
{
...
void TheTabEditor::moveCaretToStart()
{
  getCaret().moveToStartPosition();
}

void TheTabEditor::moveCaretRight()
{
  getCaret().moveHorizontal(1);
}

void TheTabEditor::moveCaretLeft()
{
  getCaret().moveHorizontal(-1);
}

void TheTabEditor::moveCaretDown()
{
  getCaret().moveVertical(1);
}

void TheTabEditor::moveCaretUp()
{
  getCaret().moveVertical(-1);
}

void TheTabEditor::moveCaretToEnd()
{
  getCaret().moveToEndPosition();
}

void TheTabEditor::moveCaretToPosition(int position)
{
  getCaret().moveToPosition(position);
}

void TheTabEditor::moveCaretToFirstSection()
{
  getCaret().moveToFirstSystem();
}

void TheTabEditor::moveCaretToNextSection()
{
  getCaret().moveSystem(1);
}

void TheTabEditor::moveCaretToPrevSection()
{
  getCaret().moveSystem(-1);
}

void TheTabEditor::moveCaretToLastSection()
{
  getCaret().moveToLastSystem();
}

void TheTabEditor::moveCaretToSystem(int system)
{
  getCaret().moveToSystem(system, true);
}

void TheTabEditor::moveCaretToNextStaff()
{
  getCaret().moveStaff(1);
}

void TheTabEditor::moveCaretToPrevStaff()
{
  getCaret().moveStaff(-1);
}

void TheTabEditor::moveCaretToNextBar()
{
...
void TheTabEditor::moveCaretToPrevBar()
{
...
void TheTabEditor::shiftForward()
{
  ScoreLocation& location = getLocation();
  myUndoManager->push(new ShiftPositions(location, ShiftPositions::Forward), location.getSystemIndex());
}

void TheTabEditor::shiftBackward()
{
  ScoreLocation& location = getLocation();
  myUndoManager->push(new ShiftPositions(location, ShiftPositions::Backward), location.getSystemIndex());
}

void TheTabEditor::removeCurrentPosition()
{
  ...
  if (!isNote) {
    ...
    myUndoManager->push(new RemoveNote(location), location.getSystemIndex());
  }
}

void TheTabEditor::removeSelectedPositions()
{
  ScoreLocation location(getLocation());

  const std::vector<Position*> selectedPositions(location.getSelectedPositions());
  ...
  std::transform(selectedPositions.begin(),
  ...
  for (int position : positions) {
    ...
    myUndoManager->push(new RemovePosition(location), location.getSystemIndex());
  }

  std::vector<int> barPositions;
  std::transform(
    bars.begin(), bars.end(), std::back_inserter(barPositions), std::mem_fun(&Barline::getPosition));

  // Remove each of the selected barlines.
  for (int position : barPositions) {
    location.setPositionIndex(position);
    myUndoManager->push(new RemoveBarline(location), location.getSystemIndex());
  }

  myUndoManager->endMacro();
}

void TheTabEditor::gotoBarline()
{
...
void TheTabEditor::gotoRehearsalSign()
{
...
void TheTabEditor::editChordName()
{
  ScoreLocation& location(getLocation());

  if (!ScoreUtils::findByPosition(location.getSystem().getChords(), location.getPositionIndex())) {
    ChordNameDialog dialog(this);
    if (dialog.exec() == QDialog::Accepted) {
      ChordText text(location.getPositionIndex(), dialog.getChordName());

      myUndoManager->push(new AddChordText(location, text), location.getSystemIndex());
    } else
      myChordNameCommand->setChecked(false);
  } else {
    myUndoManager->push(new RemoveChordText(location), location.getSystemIndex());
  }
}

void TheTabEditor::editTextItem()
{
  ScoreLocation& location(getLocation());

  if (!ScoreUtils::findByPosition(location.getSystem().getTextItems(), location.getPositionIndex())) {
    TextItemDialog dialog(this);
    if (dialog.exec() == QDialog::Accepted) {
      TextItem text(location.getPositionIndex(), dialog.getContents());

      myUndoManager->push(new AddTextItem(location, text), location.getSystemIndex());
    } else
      myTextCommand->setChecked(false);
  } else {
    myUndoManager->push(new RemoveTextItem(location), location.getSystemIndex());
  }
}

void TheTabEditor::insertSystemAtEnd()
{
  insertSystem(static_cast<int>(getLocation().getScore().getSystems().size()));
}

void TheTabEditor::insertSystemBefore()
{
  insertSystem(getLocation().getSystemIndex());
}

void TheTabEditor::insertSystemAfter()
{
  insertSystem(getLocation().getSystemIndex() + 1);
}

void TheTabEditor::removeCurrentSystem()
{
  ScoreLocation& location = getLocation();
  myUndoManager->push(new RemoveSystem(location.getScore(), location.getSystemIndex()),
                      UndoManager::AFFECTS_ALL_SYSTEMS);
}

void TheTabEditor::insertStaffBefore()
{
  insertStaff(getLocation().getStaffIndex());
}

void TheTabEditor::insertStaffAfter()
{
...
void TheTabEditor::updateNoteDuration(Position::DurationType duration)
{
  // Set the duration for future notes that are added.
  myActiveDurationType = duration;

  if (!getLocation().getSelectedPositions().empty()) {
    myUndoManager->push(new EditNoteDuration(getLocation(), duration, false), getLocation().getSystemIndex());
  } else
    updateCommands();
}

static Position::DurationType changeDuration(Position::DurationType duration, bool increase)
{
  if ((increase && duration == Position::WholeNote) || (!increase && duration == Position::SixtyFourthNote)) {
    return duration;
  }

  return static_cast<Position::DurationType>(increase ? duration / 2 : duration * 2);
}

void TheTabEditor::changeNoteDuration(bool increase)
{
  std::vector<Position*> selected_positions = getLocation().getSelectedPositions();
  ...
  if (selected_positions.empty()) {
    ...
    for (const Position* pos : selected_positions) {
      ...
      Position::DurationType new_duration = changeDuration(pos->getDurationType(), increase);
      ...
      myUndoManager->push(new EditNoteDuration(location, new_duration, false), location.getSystemIndex());
    }
...
void TheTabEditor::addDot()
{
  ...
  if (pos->hasProperty(Position::Dotted)) {
    myUndoManager->push(
      new AddPositionProperty(location, Position::DoubleDotted, myDoubleDottedCommand->text()),
      location.getSystemIndex());
  } else {
    myUndoManager->push(new AddPositionProperty(location, Position::Dotted, myDottedCommand->text()),
                        location.getSystemIndex());
  }
}

void TheTabEditor::removeDot()
{
  ScoreLocation& location = getLocation();
  const Position* pos = location.getPosition();
  Q_ASSERT(pos);
  ...
  if (pos->hasProperty(Position::DoubleDotted)) {
    ...
    myUndoManager->push(new RemovePositionProperty(location, Position::Dotted, myDottedCommand->text()),
                        location.getSystemIndex());
  }
}
...
void TheTabEditor::editTiedNote()
{
  ...
  if (!location.hasSelection() && !location.getNote()) {
    ...
    const Note* prevNote = VoiceUtils::getPreviousNote(voice, location.getPositionIndex(), string);
    if (prevNote) {
      Note newNote(*prevNote);
      newNote.setProperty(Note::Tied);
      myUndoManager->push(new AddNote(location, newNote, myActiveDurationType), location.getSystemIndex());
    } else
      myTieCommand->setChecked(false);
  } else {
    if (!location.hasSelection()) {
      const Note* note = location.getNote();
      if (!VoiceUtils::canTieNote(voice, location.getPositionIndex(), *note)) {
        myTieCommand->setChecked(false);
        return;
      }
    } else {
...
void TheTabEditor::editIrregularGrouping(bool setAsTriplet)
{
  ScoreLocation& location = getLocation();
  ...
  if (selectedPositions.size() == 1) {
    ...
    auto groups = VoiceUtils::getIrregularGroupsInRange(location.getVoice(), position, position);
    if (!groups.empty()) {
      myUndoManager->push(new RemoveIrregularGrouping(location, *groups.back()), location.getSystemIndex());
    }
    return;
  } else {
    IrregularGrouping group(
      selectedPositions.front()->getPosition(), static_cast<int>(selectedPositions.size()), 3, 2);

    if (setAsTriplet) {
      myUndoManager->push(new AddIrregularGrouping(location, group), location.getSystemIndex());
    } else {
...
void TheTabEditor::addRest()
{
  ScoreLocation& location = getLocation();
  const Position* pos = location.getPosition();
  const Position::DurationType duration = pos ? pos->getDurationType() : myActiveDurationType;

  myUndoManager->push(new AddRest(location, duration), location.getSystemIndex());
}
...
void TheTabEditor::editMultiBarRest()
{
  ...
  if (location.getPosition() && location.getPosition()->hasMultiBarRest()) {
    ...
    // Verify that the bar is empty.
    {
      ...
      const Barline* nextBar = system.getNextBarline(location.getPositionIndex());

      if (!ScoreUtils::findInRange(
             location.getVoice().getPositions(), prevBar->getPosition(), nextBar->getPosition())
             .empty()) {
        QMessageBox message(this);
        message.setText(tr("Cannot add a multi-bar rest to a non-empty measure."));
        message.exec();
    ...
    if (dialog.exec() == QDialog::Accepted) {
      myUndoManager->push(new AddMultiBarRest(location, dialog.getBarCount()), location.getSystemIndex());
    } else
      myMultibarRestCommand->setChecked(false);
  }
}
...
void TheTabEditor::editRehearsalSign()
{
  ...
  if (barline->hasRehearsalSign()) {
    myUndoManager->push(new RemoveRehearsalSign(location), UndoManager::AFFECTS_ALL_SYSTEMS);
  } else {
    RehearsalSignDialog dialog(this);

    if (dialog.exec() == QDialog::Accepted) {
      myUndoManager->push(new AddRehearsalSign(location, dialog.getDescription()),
                          UndoManager::AFFECTS_ALL_SYSTEMS);
    } else
...
void TheTabEditor::editTempoMarker()
{
  ...
  if (marker) {
    myUndoManager->push(new RemoveTempoMarker(location), location.getSystemIndex());
  } else {
    ...
    if (dialog.exec() == QDialog::Accepted) {
      ...
      myUndoManager->push(new AddTempoMarker(location, marker), location.getSystemIndex());
    } else
...
void TheTabEditor::editKeySignatureFromCaret()
{
  editKeySignature(getLocation());
}

void TheTabEditor::editTimeSignatureFromCaret()
{
  editTimeSignature(getLocation());
}

void TheTabEditor::insertStandardBarline()
{
  ScoreLocation& location = getLocation();
  myUndoManager->push(new AddBarline(location, Barline(location.getPositionIndex(), Barline::SingleBar)),
                      location.getSystemIndex());
}

void TheTabEditor::editBarlineFromCaret()
{
...
void TheTabEditor::editMusicalDirection()
{
  ...
  if (direction) {
    myUndoManager->push(new RemoveDirection(location), location.getSystemIndex());
  } else {
    ...
    if (dialog.exec() == QDialog::Accepted) {
      ...
      myUndoManager->push(new AddDirection(location, direction), location.getSystemIndex());
    } else
      myDirectionCommand->setChecked(false);
  }
}

void TheTabEditor::editRepeatEnding()
{
  ScoreLocation& location(getLocation());
  ...
  if (!ending) {
    ...
    if (dialog.exec() == QDialog::Accepted) {
      ...
      myUndoManager->push(new AddAlternateEnding(location, altending), location.getSystemIndex());
    } else
      myRepeatEndingCommand->setChecked(false);
  } else {
    myUndoManager->push(new RemoveAlternateEnding(location), location.getSystemIndex());
  }
}

void TheTabEditor::editDynamic()
{
  ScoreLocation& location = getLocation();
  ...
  if (dynamic) {
    myUndoManager->push(new RemoveDynamic(location), location.getSystemIndex());
  } else {
    DynamicDialog dialog(this);
    if (dialog.exec() == QDialog::Accepted) {
      Dynamic dynamic(location.getPositionIndex(), dialog.getVolumeLevel());

      myUndoManager->push(new AddDynamic(location, dynamic), location.getSystemIndex());
    } else
...
void TheTabEditor::editArtificialHarmonic()
{
  auto& location = getLocation();
  ...
  if (!location.getNote()->hasArtificialHarmonic()) {
    ...
    if (dialog.exec() == QDialog::Accepted) {
      myUndoManager->push(new AddArtificialHarmonic(location, dialog.getHarmonic()),
                          location.getSystemIndex());
    } else
      myArtificialHarmonicCommand->setChecked(false);
  } else {
    myUndoManager->push(new RemoveArtificialHarmonic(location), location.getSystemIndex());
  }
}
...
void TheTabEditor::editTappedHarmonic()
{
  ...
  if (note->hasTappedHarmonic())
    myUndoManager->push(new RemoveTappedHarmonic(location), location.getSystemIndex());
  else {
    TappedHarmonicDialog dialog(this, note->getFretNumber());
    if (dialog.exec() == QDialog::Accepted) {
      myUndoManager->push(new AddTappedHarmonic(location, dialog.getTappedFret()), location.getSystemIndex());
    } else
...
void TheTabEditor::editBend()
{
  ...
  if (note->hasBend()) {
    myUndoManager->push(new RemoveBend(location), location.getSystemIndex());
  } else {
    BendDialog dialog(this);
    if (dialog.exec() == QDialog::Accepted) {
      myUndoManager->push(new AddBend(location, dialog.getBend()), location.getSystemIndex());
    } else
...
void TheTabEditor::editTrill()
{
  ...
  if (note->hasTrill())
    ...
    if (dialog.exec() == QDialog::Accepted) {
      myUndoManager->push(new AddTrill(location, dialog.getTrilledFret()), location.getSystemIndex());
    } else
...
void TheTabEditor::editLeftHandFingering()
{
  ...
  if (note->hasLeftHandFingering()) {
    myUndoManager->push(new RemoveLeftHandFingering(location), location.getSystemIndex());
  } else {
    LeftHandFingeringDialog dialog(this);
    if (dialog.exec() == QDialog::Accepted) {
      myUndoManager->push(new AddLeftHandFingering(location, dialog.getLeftHandFingering()),
                          location.getSystemIndex());
    } else
...
void TheTabEditor::editPlayerChange()
{
  ...
  if (ScoreUtils::findByPosition(location.getSystem().getPlayerChanges(), location.getPositionIndex())) {
    myUndoManager->push(new RemovePlayerChange(location), UndoManager::AFFECTS_ALL_SYSTEMS);
  } else {
    ...
    const PlayerChange* currentPlayers = ScoreUtils::getCurrentPlayers(
      location.getScore(), location.getSystemIndex(), location.getPositionIndex());

    PlayerChangeDialog dialog(this, location.getScore(), location.getSystem(), currentPlayers);
    if (dialog.exec() == QDialog::Accepted) {
      myUndoManager->push(new AddPlayerChange(location, dialog.getPlayerChange()),
                          UndoManager::AFFECTS_ALL_SYSTEMS);
    } else
...
void TheTabEditor::editPlayer(int playerIndex, const Player& player, bool undoable)
{
  ...
  if (!undoable)
    ...
    myUndoManager->push(new EditPlayer(location.getScore(), playerIndex, player),
                        UndoManager::AFFECTS_ALL_SYSTEMS);
  }
}

void TheTabEditor::removePlayer(int index)
{
  ScoreLocation& location = getLocation();

  myUndoManager->push(new RemovePlayer(location.getScore(), index), UndoManager::AFFECTS_ALL_SYSTEMS);
}

void TheTabEditor::showTuningDictionary()
{
  TuningDictionaryDialog dialog(this, *myTuningDictionary);
  dialog.exec();
}

void TheTabEditor::editViewFilters()
{
  ViewFilterDialog dialog(this);
  ViewFilterPresenter presenter(dialog, getLocation().getScore());
  if (presenter.exec()) {
    myUndoManager->push(new EditViewFilters(getLocation().getScore(), presenter.getFilters()),
                        UndoManager::AFFECTS_ALL_SYSTEMS);
  }
}

bool TheTabEditor::eventFilter(QObject* object, QEvent* event)
{
  ...
  if (scorearea && event->type() == QEvent::KeyPress) {
    ...
    if (keyEvent->key() >= Qt::Key_0 && keyEvent->key() <= Qt::Key_9) {
      ...
      if (!location.getBarline() || location.getPositionIndex() == 0) {
        ...
        if (location.getNote()) {
          myUndoManager->push(new EditTabNumber(location, number), location.getSystemIndex());
        } else {
          myUndoManager->push(new AddNote(location, Note(location.getString(), number), myActiveDurationType),
                              location.getSystemIndex());
        }
...
void TheTabEditor::closeEvent(QCloseEvent* event)
{
...
void TheTabEditor::updateWindowTitle()
{
  QString name;
...
void TheTabEditor::createCommands()
{
  // File-related commands.
  myNewDocumentCommand = new Command(tr("&New"), "File.New", QKeySequence::New, this);
  connect(myNewDocumentCommand, SIGNAL(triggered()), this, SLOT(createNewDocument()));

  myOpenFileCommand = new Command(tr("&Open..."), "File.Open", QKeySequence::Open, this);
  connect(myOpenFileCommand, SIGNAL(triggered()), this, SLOT(openFile()));

  myCloseTabCommand = new Command(tr("&Close Tab"), "File.CloseTab", Qt::CTRL + Qt::Key_W, this);
  connect(myCloseTabCommand, SIGNAL(triggered()), this, SLOT(closeCurrentTab()));

  mySaveCommand = new Command(tr("Save"), "File.Save", QKeySequence::Save, this);
  connect(mySaveCommand, SIGNAL(triggered()), this, SLOT(saveFile()));

  mySaveAsCommand = new Command(tr("Save As..."), "File.SaveAs", QKeySequence::SaveAs, this);
  connect(mySaveAsCommand, SIGNAL(triggered()), this, SLOT(saveFileAs()));

  myPrintCommand = new Command(tr("Print..."), "File.Print", QKeySequence::Print, this);
  connect(myPrintCommand, SIGNAL(triggered()), this, SLOT(printDocument()));

  myPrintPreviewCommand = new Command(tr("Print Preview..."), "File.PrintPreview", QKeySequence(), this);
  connect(myPrintPreviewCommand, SIGNAL(triggered()), this, SLOT(printPreview()));

  myEditShortcutsCommand =
    new Command(tr("Customize Shortcuts..."), "File.CustomizeShortcuts", QKeySequence(), this);
  connect(myEditShortcutsCommand, SIGNAL(triggered()), this, SLOT(editKeyboardShortcuts()));

  myEditPreferencesCommand =
    new Command(tr("&Preferences..."), "File.Preferences", QKeySequence::Preferences, this);
  connect(myEditPreferencesCommand, SIGNAL(triggered()), this, SLOT(editPreferences()));

  myExitCommand = new Command(tr("&Quit"), "File.Quit", QKeySequence::Quit, this);
  ...
  myCopyCommand = new Command(tr("Copy"), "Edit.Copy", QKeySequence::Copy, this);
  connect(myCopyCommand, SIGNAL(triggered()), this, SLOT(copySelectedNotes()));

  myPasteCommand = new Command(tr("Paste"), "Edit.Paste", QKeySequence::Paste, this);
  connect(myPasteCommand, SIGNAL(triggered()), this, SLOT(pasteNotes()));

  myPolishCommand =
    new Command(tr("Polish Score"), "Edit.PolishScore", QKeySequence(Qt::SHIFT + Qt::Key_J), this);
  connect(myPolishCommand, &QAction::triggered, this, &TheTabEditor::polishScore);

  myPolishSystemCommand =
    new Command(tr("Polish System"), "Edit.PolishSystem", QKeySequence(Qt::Key_J), this);
  connect(myPolishSystemCommand, &QAction::triggered, this, &TheTabEditor::polishSystem);

  // File Information
  myFileInfoCommand = new Command(tr("File Information..."), "Edit.FileInformation", QKeySequence(), this);
  connect(myFileInfoCommand, SIGNAL(triggered()), this, SLOT(editFileInformation()));

  // Playback-related actions.
  myPlayPauseCommand = new Command(tr("Play"), "Playback.PlayPause", Qt::Key_Space, this);
  connect(myPlayPauseCommand, SIGNAL(triggered()), this, SLOT(startStopPlayback()));
  ...
  myPlayFromStartOfMeasureCommand =
    new Command(tr("Play From Start Of Measure"), "Playback.PlayFromStartOfMeasure", play_start_seq, this);
  connect(myPlayFromStartOfMeasureCommand, &QAction::triggered, [this]() {
    startStopPlayback(/* from_measure_start */ true);
  });

  myStopCommand = new Command(tr("Stop"), "Playback.Stop", Qt::ALT + Qt::Key_Space, this);
  connect(myStopCommand, &QAction::triggered, this, &TheTabEditor::stopPlayback);

  myRewindCommand = new Command(tr("Rewind"), "Playback.Rewind", Qt::ALT + Qt::Key_Left, this);
  connect(myRewindCommand, &QAction::triggered, this, &TheTabEditor::rewindPlaybackToStart);

  myMetronomeCommand = new Command(tr("Metronome"), "Playback.Metronome", QKeySequence(), this);
  myMetronomeCommand->setCheckable(true);
  connect(myMetronomeCommand, &QAction::triggered, this, &TheTabEditor::toggleMetronome);

  // Section navigation actions.
  myFirstSectionCommand = new Command(
    tr("First Section"), "Position.Section.FirstSection", QKeySequence::MoveToStartOfDocument, this);
  connect(myFirstSectionCommand, SIGNAL(triggered()), this, SLOT(moveCaretToFirstSection()));

  myNextSectionCommand =
    new Command(tr("Next Section"), "Position.Section.NextSection", QKeySequence::MoveToNextPage, this);
  connect(myNextSectionCommand, SIGNAL(triggered()), this, SLOT(moveCaretToNextSection()));

  myPrevSectionCommand = new Command(
    tr("Previous Section"), "Position.Section.PreviousSection", QKeySequence::MoveToPreviousPage, this);
  connect(myPrevSectionCommand, SIGNAL(triggered()), this, SLOT(moveCaretToPrevSection()));

  myLastSectionCommand =
    new Command(tr("Last Section"), "Position.Section.LastSection", QKeySequence::MoveToEndOfDocument, this);
  connect(myLastSectionCommand, SIGNAL(triggered()), this, SLOT(moveCaretToLastSection()));

  myShiftForwardCommand =
    new Command(tr("Shift Forward"), "Position.ShiftForward", QKeySequence(Qt::Key_Insert), this);
  connect(myShiftForwardCommand, SIGNAL(triggered()), this, SLOT(shiftForward()));

  myShiftBackwardCommand = new Command(
    tr("Shift Backward"), "Position.ShiftBackward", QKeySequence(Qt::SHIFT + Qt::Key_Insert), this);
  connect(myShiftBackwardCommand, SIGNAL(triggered()), this, SLOT(shiftBackward()));
  ...
  myStartPositionCommand =
    new Command(tr("Move to &Start"), "Position.Staff.MoveToStart", move_start_seq, this);
  connect(myStartPositionCommand, SIGNAL(triggered()), this, SLOT(moveCaretToStart()));

  myNextPositionCommand =
    new Command(tr("&Next Position"), "Position.Staff.NextPosition", QKeySequence::MoveToNextChar, this);
  connect(myNextPositionCommand, SIGNAL(triggered()), this, SLOT(moveCaretRight()));

  myPrevPositionCommand = new Command(
    tr("&Previous Position"), "Position.Staff.PreviousPosition", QKeySequence::MoveToPreviousChar, this);
  connect(myPrevPositionCommand, SIGNAL(triggered()), this, SLOT(moveCaretLeft()));

  myNextStringCommand =
    new Command(tr("Next String"), "Position.Staff.NextString", QKeySequence::MoveToNextLine, this);
  connect(myNextStringCommand, SIGNAL(triggered()), this, SLOT(moveCaretDown()));

  myPrevStringCommand = new Command(
    tr("Previous String"), "Position.Staff.PreviousString", QKeySequence::MoveToPreviousLine, this);
  connect(myPrevStringCommand, SIGNAL(triggered()), this, SLOT(moveCaretUp()));

  myLastPositionCommand = new Command(tr("Move to &End"), "Position.Staff.MoveToEnd", move_end_seq, this);
  connect(myLastPositionCommand, SIGNAL(triggered()), this, SLOT(moveCaretToEnd()));

  myNextStaffCommand =
    new Command(tr("Next Staff"), "Position.Staff.NextStaff", Qt::ALT + Qt::Key_Down, this);
  connect(myNextStaffCommand, SIGNAL(triggered()), this, SLOT(moveCaretToNextStaff()));

  myPrevStaffCommand =
    new Command(tr("Previous Staff"), "Position.Staff.PreviousStaff", Qt::ALT + Qt::Key_Up, this);
  connect(myPrevStaffCommand, SIGNAL(triggered()), this, SLOT(moveCaretToPrevStaff()));

  myNextBarCommand = new Command(tr("Next Bar"), "Position.Staff.NextBar", Qt::Key_Tab, this);
  connect(myNextBarCommand, SIGNAL(triggered()), this, SLOT(moveCaretToNextBar()));

  myPrevBarCommand =
    new Command(tr("Previous Bar"), "Position.Staff.PreviousBar", Qt::SHIFT + Qt::Key_Tab, this);
  connect(myPrevBarCommand, SIGNAL(triggered()), this, SLOT(moveCaretToPrevBar()));

  myRemoveNoteCommand = new Command(tr("Remove Note"), "Position.RemoveNote", QKeySequence::Delete, this);
  connect(myRemoveNoteCommand, SIGNAL(triggered()), this, SLOT(removeCurrentPosition()));

  myRemovePositionCommand =
    new Command(tr("Remove Position"), "Position.RemovePosition", QKeySequence::DeleteEndOfWord, this);
  connect(myRemovePositionCommand, SIGNAL(triggered()), this, SLOT(removeSelectedPositions()));

  myGoToBarlineCommand =
    new Command(tr("Go To Barline..."), "Position.GoToBarline", Qt::CTRL + Qt::Key_G, this);
  connect(myGoToBarlineCommand, SIGNAL(triggered()), this, SLOT(gotoBarline()));

  myGoToRehearsalSignCommand =
    new Command(tr("Go To Rehearsal Sign..."), "Position.GoToRehearsalSign", Qt::CTRL + Qt::Key_H, this);
  connect(myGoToRehearsalSignCommand, SIGNAL(triggered()), this, SLOT(gotoRehearsalSign()));

  // Text-related actions.
  myChordNameCommand = new Command(tr("Chord Name..."), "Text.ChordName", Qt::Key_C, this);
  myChordNameCommand->setCheckable(true);
  connect(myChordNameCommand, SIGNAL(triggered()), this, SLOT(editChordName()));

  myTextCommand = new Command(tr("Text..."), "Text.TextItem", QKeySequence(), this);
  myTextCommand->setCheckable(true);
  connect(myTextCommand, &QAction::triggered, this, &TheTabEditor::editTextItem);

  myInsertSystemAtEndCommand =
    new Command(tr("Insert System At End"), "Section.InsertSystemAtEnd", Qt::Key_N, this);
  connect(myInsertSystemAtEndCommand, SIGNAL(triggered()), this, SLOT(insertSystemAtEnd()));

  myInsertSystemBeforeCommand = new Command(tr("Insert System Before"),
                                            "Section.InsertSystemBefore",
                                            QKeySequence(Qt::ALT + Qt::SHIFT + Qt::Key_N),
                                            this);
  connect(myInsertSystemBeforeCommand, SIGNAL(triggered()), this, SLOT(insertSystemBefore()));

  myInsertSystemAfterCommand = new Command(
    tr("Insert System After"), "Section.InsertSystemAfter", QKeySequence(Qt::SHIFT + Qt::Key_N), this);
  connect(myInsertSystemAfterCommand, SIGNAL(triggered()), this, SLOT(insertSystemAfter()));

  myRemoveCurrentSystemCommand = new Command(tr("Remove Current System"),
                                             "Section.RemoveCurrentSystem",
                                             QKeySequence(Qt::CTRL + Qt::SHIFT + Qt::Key_N),
                                             this);
  connect(myRemoveCurrentSystemCommand, SIGNAL(triggered()), this, SLOT(removeCurrentSystem()));

  myInsertStaffBeforeCommand =
    new Command(tr("Insert Staff Before"), "Section.InsertStaffBefore", QKeySequence(), this);
  connect(myInsertStaffBeforeCommand, &QAction::triggered, this, &TheTabEditor::insertStaffBefore);

  myInsertStaffAfterCommand =
    new Command(tr("Insert Staff After"), "Section.InsertStaffAfter", QKeySequence(), this);
  connect(myInsertStaffAfterCommand, &QAction::triggered, this, &TheTabEditor::insertStaffAfter);

  myRemoveCurrentStaffCommand =
    new Command(tr("Remove Current Staff"), "Section.RemoveCurrentStaff", QKeySequence(), this);
  connect(myRemoveCurrentStaffCommand, &QAction::triggered, this, &TheTabEditor::removeCurrentStaff);

  myIncreaseLineSpacingCommand =
    new Command(tr("Increase"), "Section.LineSpacing.Increase", QKeySequence(), this);
  connect(myIncreaseLineSpacingCommand, &QAction::triggered, [=]() { adjustLineSpacing(1); });

  myDecreaseLineSpacingCommand =
    new Command(tr("Decrease"), "Section.LineSpacing.Decrease", QKeySequence(), this);
  connect(myDecreaseLineSpacingCommand, &QAction::triggered, [=]() { adjustLineSpacing(-1); });

  // Note-related actions.
  myNoteDurationGroup = new QActionGroup(this);
  createNoteDurationCommand(myWholeNoteCommand, tr("Whole"), "Notes.WholeNote", Position::WholeNote);
  createNoteDurationCommand(myHalfNoteCommand, tr("Half"), "Notes.HalfNote", Position::HalfNote);
  createNoteDurationCommand(myQuarterNoteCommand, tr("Quarter"), "Notes.QuarterNote", Position::QuarterNote);
  createNoteDurationCommand(myEighthNoteCommand, tr("8th"), "Notes.EighthNote", Position::EighthNote);
  createNoteDurationCommand(
    mySixteenthNoteCommand, tr("16th"), "Notes.SixteenthNote", Position::SixteenthNote);
  createNoteDurationCommand(
    myThirtySecondNoteCommand, tr("32nd"), "Notes.ThirtySecondNote", Position::ThirtySecondNote);
  createNoteDurationCommand(
    mySixtyFourthNoteCommand, tr("64th"), "Notes.SixtyFourthNote", Position::SixtyFourthNote);

  myIncreaseDurationCommand =
    new Command(tr("Increase Duration"), "Notes.Duration.Increase", Qt::SHIFT + Qt::Key_Up, this);
  connect(myIncreaseDurationCommand, &QAction::triggered, [=]() { changeNoteDuration(true); });

  myDecreaseDurationCommand =
    new Command(tr("Decrease Duration"), "Notes.Duration.Decrease", Qt::SHIFT + Qt::Key_Down, this);
  connect(myDecreaseDurationCommand, &QAction::triggered, [=]() { changeNoteDuration(false); });

  createPositionPropertyCommand(
    myDottedCommand, tr("Dotted"), "Notes.Dotted", QKeySequence(), Position::Dotted);

  createPositionPropertyCommand(
    myDoubleDottedCommand, tr("Double Dotted"), "Notes.DoubleDotted", QKeySequence(), Position::DoubleDotted);

  myAddDotCommand = new Command(tr("Add Dot"), "Notes.Dot.Add", Qt::SHIFT + Qt::Key_Right, this);
  connect(myAddDotCommand, SIGNAL(triggered()), this, SLOT(addDot()));

  myRemoveDotCommand = new Command(tr("Remove Dot"), "Notes.Dot.Remove", Qt::SHIFT + Qt::Key_Left, this);
  connect(myRemoveDotCommand, SIGNAL(triggered()), this, SLOT(removeDot()));

  myLeftHandFingeringCommand =
    new Command(tr("Left Hand Fingering..."), "Notes.LeftHandFingering", QKeySequence(), this);
  myLeftHandFingeringCommand->setCheckable(true);
  connect(myLeftHandFingeringCommand, &QAction::triggered, this, &TheTabEditor::editLeftHandFingering);
  ...
  createNotePropertyCommand(myMutedCommand, tr("Muted"), "Notes.Muted", Qt::Key_X, Note::Muted);
  createNotePropertyCommand(
    myGhostNoteCommand, tr("Ghost Note"), "Notes.GhostNote", Qt::Key_G, Note::GhostNote);

  createPositionPropertyCommand(
    myFermataCommand, tr("Fermata"), "Notes.Fermata", Qt::Key_F, Position::Fermata);

  createPositionPropertyCommand(
    myLetRingCommand, tr("Let Ring"), "Notes.LetRing", QKeySequence(), Position::LetRing);

  createPositionPropertyCommand(
    myGraceNoteCommand, tr("Grace Note"), "Notes.GraceNote", QKeySequence(), Position::Acciaccatura);

  createPositionPropertyCommand(
    myStaccatoCommand, tr("Staccato"), "Notes.Staccato", Qt::Key_Z, Position::Staccato);

  createPositionPropertyCommand(myMarcatoCommand, tr("Accent"), "Notes.Accent", Qt::Key_A, Position::Marcato);

  createPositionPropertyCommand(
    mySforzandoCommand, tr("Heavy Accent"), "Notes.HeavyAccent", QKeySequence(), Position::Sforzando);

  // Octave actions
  createNotePropertyCommand(
    myOctave8vaCommand, tr("8va"), "Notes.Octave.8va", QKeySequence(), Note::Octave8va);
  createNotePropertyCommand(
    myOctave15maCommand, tr("15ma"), "Notes.Octave.15ma", QKeySequence(), Note::Octave15ma);
  createNotePropertyCommand(
    myOctave8vbCommand, tr("8vb"), "Notes.Octave.8vb", QKeySequence(), Note::Octave8vb);
  createNotePropertyCommand(
    myOctave15mbCommand, tr("15mb"), "Notes.Octave.15mb", QKeySequence(), Note::Octave15mb);

  myTripletCommand = new Command(tr("Triplet"), "Notes.Triplet", Qt::Key_E, this);
  connect(myTripletCommand, &QAction::triggered, [=]() { editIrregularGrouping(true); });

  myIrregularGroupingCommand =
    new Command(tr("Irregular Grouping"), "Notes.IrregularGrouping", Qt::Key_I, this);
  connect(myIrregularGroupingCommand, &QAction::triggered, [=]() { editIrregularGrouping(false); });

  // Rest Actions.
  myRestDurationGroup = new QActionGroup(this);

  createRestDurationCommand(myWholeRestCommand, tr("Whole"), "Rests.Whole", Position::WholeNote);
  createRestDurationCommand(myHalfRestCommand, tr("Half"), "Rests.Half", Position::HalfNote);
  createRestDurationCommand(myQuarterRestCommand, tr("Quarter"), "Rests.Quarter", Position::QuarterNote);
  createRestDurationCommand(myEighthRestCommand, tr("8th"), "Rests.Eighth", Position::EighthNote);
  createRestDurationCommand(mySixteenthRestCommand, tr("16th"), "Rests.Sixteenth", Position::SixteenthNote);
  createRestDurationCommand(
    myThirtySecondRestCommand, tr("32nd"), "Rests.ThirtySecond", Position::ThirtySecondNote);
  createRestDurationCommand(
    mySixtyFourthRestCommand, tr("64th"), "Rests.SixtyFourth", Position::SixtyFourthNote);

  myAddRestCommand = new Command(tr("Add Rest"), "Rests.AddRest", Qt::Key_R, this);
  connect(myAddRestCommand, SIGNAL(triggered()), this, SLOT(addRest()));

  myMultibarRestCommand = new Command(tr("Multi-Bar Rest..."), "Rests.MultibarRest", QKeySequence(), this);
  myMultibarRestCommand->setCheckable(true);
  connect(myMultibarRestCommand, SIGNAL(triggered()), this, SLOT(editMultiBarRest()));

  // Music Symbol Actions
  myRehearsalSignCommand =
    new Command(tr("Rehearsal Sign..."), "MusicSymbols.RehearsalSign", Qt::SHIFT + Qt::Key_R, this);
  myRehearsalSignCommand->setCheckable(true);
  connect(myRehearsalSignCommand, SIGNAL(triggered()), this, SLOT(editRehearsalSign()));

  myTempoMarkerCommand = new Command(tr("Tempo Marker..."), "MusicSymbols.TempoMarker", Qt::Key_O, this);
  myTempoMarkerCommand->setCheckable(true);
  connect(myTempoMarkerCommand, SIGNAL(triggered()), this, SLOT(editTempoMarker()));

  myAlterationOfPaceCommand =
    new Command(tr("Alteration of Pace..."), "MusicSymbols.AlterationOfPace", QKeySequence(), this);
  myAlterationOfPaceCommand->setCheckable(true);
  connect(myAlterationOfPaceCommand, SIGNAL(triggered()), this, SLOT(editAlterationOfPace()));

  myKeySignatureCommand =
    new Command(tr("Edit Key Signature..."), "MusicSymbols.EditKeySignature", Qt::Key_K, this);
  connect(myKeySignatureCommand, SIGNAL(triggered()), this, SLOT(editKeySignatureFromCaret()));

  myTimeSignatureCommand =
    new Command(tr("Edit Time Signature..."), "MusicSymbols.EditTimeSignature", Qt::Key_T, this);
  connect(myTimeSignatureCommand, SIGNAL(triggered()), this, SLOT(editTimeSignatureFromCaret()));

  myStandardBarlineCommand =
    new Command(tr("Insert Standard Barline"), "MusicSymbols.InsertStandardBarline", Qt::Key_B, this);
  connect(myStandardBarlineCommand, SIGNAL(triggered()), this, SLOT(insertStandardBarline()));

  myBarlineCommand = new Command(tr("Barline..."), "MusicSymbols.Barline", Qt::SHIFT + Qt::Key_B, this);
  connect(myBarlineCommand, SIGNAL(triggered()), this, SLOT(editBarlineFromCaret()));

  myDirectionCommand =
    new Command(tr("Musical Direction..."), "MusicSymbols.MusicalDirection", Qt::SHIFT + Qt::Key_D, this);
  myDirectionCommand->setCheckable(true);
  connect(myDirectionCommand, SIGNAL(triggered()), this, SLOT(editMusicalDirection()));

  myRepeatEndingCommand =
    new Command(tr("Repeat Ending..."), "MusicSymbols.RepeatEnding", Qt::SHIFT + Qt::Key_E, this);
  myRepeatEndingCommand->setCheckable(true);
  connect(myRepeatEndingCommand, SIGNAL(triggered()), this, SLOT(editRepeatEnding()));

  myDynamicCommand = new Command(tr("Dynamic..."), "MusicSymbols.Dynamic", Qt::Key_D, this);
  ...
  // Tab Symbol Actions.
  myHammerPullCommand = new Command(tr("Hammer On/Pull Off"), "TabSymbols.HammerPull", Qt::Key_H, this);
  myHammerPullCommand->setCheckable(true);
  connect(myHammerPullCommand, SIGNAL(triggered()), this, SLOT(editHammerPull()));

  createNotePropertyCommand(myHammerOnFromNowhereCommand,
                            tr("Hammer On From Nowhere"),
                            "TabSymbols.HammerOnFromNowhere",
                            QKeySequence(),
                            Note::HammerOnFromNowhere);

  createNotePropertyCommand(myPullOffToNowhereCommand,
                            tr("Pull Off To Nowhere"),
                            "TabSymbols.PullOffToNowhere",
                            QKeySequence(),
                            Note::PullOffToNowhere);

  createNotePropertyCommand(myNaturalHarmonicCommand,
                            tr("Natural Harmonic"),
  ...
  myArtificialHarmonicCommand =
    new Command(tr("Artificial Harmonic..."), "TabSymbols.ArtificialHarmonic", QKeySequence(), this);
  myArtificialHarmonicCommand->setCheckable(true);
  connect(myArtificialHarmonicCommand, SIGNAL(triggered()), this, SLOT(editArtificialHarmonic()));

  myTappedHarmonicCommand =
    new Command(tr("Tapped Harmonic..."), "TabSymbols.TappedHarmonic", QKeySequence(), this);
  myTappedHarmonicCommand->setCheckable(true);
  connect(myTappedHarmonicCommand, SIGNAL(triggered()), this, SLOT(editTappedHarmonic()));

  myBendCommand = new Command(tr("Bend..."), "TabSymbols.Bend", QKeySequence(), this);
  myBendCommand->setCheckable(true);
  connect(myBendCommand, &QAction::triggered, this, &TheTabEditor::editBend);

  createPositionPropertyCommand(
    myVibratoCommand, tr("Vibrato"), "TabSymbols.Vibrato", Qt::Key_V, Position::Vibrato);

  createPositionPropertyCommand(
    myWideVibratoCommand, tr("Wide Vibrato"), "TabSymbols.WideVibrato", Qt::Key_W, Position::WideVibrato);

  createPositionPropertyCommand(
    myPalmMuteCommand, tr("Palm Mute"), "TabSymbols.PalmMute", Qt::Key_M, Position::PalmMuting);

  createPositionPropertyCommand(myTremoloPickingCommand,
                                tr("Tremolo Picking"),
  ...
  createPositionPropertyCommand(
    myArpeggioUpCommand, tr("Arpeggio Up"), "TabSymbols.ArpeggioUp", QKeySequence(), Position::ArpeggioUp);

  createPositionPropertyCommand(myArpeggioDownCommand,
                                tr("Arpeggio Down"),
                                "TabSymbols.ArpeggioDown",
                                QKeySequence(),
                                Position::ArpeggioDown);

  createPositionPropertyCommand(myTapCommand, tr("Tap"), "TabSymbols.Tap", Qt::Key_P, Position::Tap);

  myTrillCommand = new Command(tr("Trill..."), "TabSymbols.Trill", QKeySequence(), this);
  myTrillCommand->setCheckable(true);
  connect(myTrillCommand, SIGNAL(triggered()), this, SLOT(editTrill()));

  createPositionPropertyCommand(myPickStrokeUpCommand,
                                tr("Pickstroke Up"),
                                "TabSymbols.PickStrokeUp",
                                QKeySequence(),
                                Position::PickStrokeUp);

  createPositionPropertyCommand(myPickStrokeDownCommand,
                                tr("Pickstroke Down"),
  ...
  myShowTuningDictionaryCommand =
    new Command(tr("Tuning Dictionary..."), "Player.TuningDictionary", QKeySequence(), this);
  connect(myShowTuningDictionaryCommand, SIGNAL(triggered()), this, SLOT(showTuningDictionary()));

  myEditViewFiltersCommand =
    new Command(tr("Edit View Filters..."), "Player.EditViewFilters", QKeySequence(), this);
  connect(myEditViewFiltersCommand, &QAction::triggered, this, &TheTabEditor::editViewFilters);
  ...
  myNextTabCommand = new Command(tr("Next Tab"), "Window.NextTab", next_tab_seq, this);
  connect(myNextTabCommand, &QAction::triggered, [=]() { cycleTab(1); });

  myPrevTabCommand = new Command(tr("Previous Tab"), "Window.PreviousTab", prev_tab_seq, this);
  connect(myPrevTabCommand, &QAction::triggered, [=]() { cycleTab(-1); });

  // Help menu commands.
  myReportBugCommand = new Command(tr("Report Bug..."), "Help.ReportBug", QKeySequence(), this);
  ...
  myMixerDockWidgetCommand =
    createCommandWrapper(myMixerDockWidget->toggleViewAction(), "Window.Mixer", QKeySequence(), this);
}

void TheTabEditor::loadKeyboardShortcuts()
{
  auto settings = mySettingsManager->getReadHandle();
...
void TheTabEditor::saveKeyboardShortcuts() const
{
  auto settings = mySettingsManager->getWriteHandle();
  for (auto command : getCommands())
...
std::vector<const Command*> TheTabEditor::getCommands() const
{
  return findChildren<const Command*>().toVector().toStdVector();
}

std::vector<Command*> TheTabEditor::getCommands()
{
  return findChildren<Command*>().toVector().toStdVector();
}

void TheTabEditor::createMixer()
{
  ...
  myMixer = new Mixer(scroll, *myTuningDictionary, myPlayerEditPubSub, myPlayerRemovePubSub);
                                             ...
                                             Position::DurationType durationType)
{
  ...
  connect(command, &QAction::triggered, [=]() { updateNoteDuration(durationType); });
                                             ...
                                             Note::SimpleProperty property)
{
  ...
  command->setCheckable(true);
  connect(command, &QAction::triggered, [=]() { editSimpleNoteProperty(command, property); });
}
                                                 ...
                                                 Position::SimpleProperty property)
{
  command = new Command(menuName, commandName, shortcut, this);
  command->setCheckable(true);
  connect(command, &QAction::triggered, [=]() { editSimplePositionProperty(command, property); });
}

void TheTabEditor::createMenus()
{
  // File Menu.
...
void TheTabEditor::createTabArea()
{
  myTabWidget = new QTabWidget(this);
  myTabWidget->setDocumentMode(true);
  myTabWidget->setTabsClosable(true);

  connect(myTabWidget, SIGNAL(tabCloseRequested(int)), this, SLOT(closeTab(int)));
  connect(myTabWidget, SIGNAL(currentChanged(int)), this, SLOT(switchTab(int)));

  myPlaybackWidget =
    new PlaybackWidget(*myPlayPauseCommand, *myRewindCommand, *myStopCommand, *myMetronomeCommand, this);

  connect(myPlaybackWidget, &PlaybackWidget::activeVoiceChanged, this, &TheTabEditor::updateActiveVoice);

  connect(myPlaybackWidget, &PlaybackWidget::activeFilterChanged, this, &TheTabEditor::updateActiveFilter);

  connect(myPlaybackWidget, &PlaybackWidget::zoomChanged, this, &TheTabEditor::updateZoom);

  auto update_metronome_state = [&]() {
    auto settings = mySettingsManager->getReadHandle();
    myMetronomeCommand->setChecked(settings->get(Settings::MetronomeEnabled));
  };
...
void TheTabEditor::setPreviousDirectory(const QString& fileName)
{
  ...
  auto settings = mySettingsManager->getWriteHandle();
  settings->set(Settings::PreviousDirectory, myPreviousDirectory.toStdString());
}

void TheTabEditor::setupNewTab()
{
  auto start = std::chrono::high_resolution_clock::now();
  ...
  auto end = std::chrono::high_resolution_clock::now();
  qDebug() << "Tab opened in" << std::chrono::duration_cast<std::chrono::milliseconds>(end - start).count()
           << "ms";
}

namespace {
inline void updatePositionProperty(Command* command, const Position* pos, Position::SimpleProperty property)
{
  command->setEnabled(pos != nullptr);
  command->setChecked(pos && pos->hasProperty(property));
}

inline void updateNoteProperty(Command* command, const Note* note, Note::SimpleProperty property)
{
...
void TheTabEditor::updateCommands()
{
  if (myIsPlaying)
    return;
  ...
  const Note* note = location.getNote();
  const Barline* barline = location.getBarline();
  const TempoMarker* tempoMarker = ScoreUtils::findByPosition(system.getTempoMarkers(), position);
  const AlternateEnding* altEnding = ScoreUtils::findByPosition(system.getAlternateEndings(), position);
  ...
  myIncreaseLineSpacingCommand->setEnabled(score.getLineSpacing() < Score::MAX_LINE_SPACING);
  myDecreaseLineSpacingCommand->setEnabled(score.getLineSpacing() > Score::MIN_LINE_SPACING);
  myShiftBackwardCommand->setEnabled(!pos && (position == 0 || !barline) && !tempoMarker && !altEnding &&
                                     !dynamic);
  myRemoveNoteCommand->setEnabled(pos || barline || hasSelection);
  myRemovePositionCommand->setEnabled(pos || barline || hasSelection);

  myChordNameCommand->setChecked(ScoreUtils::findByPosition(system.getChords(), position) != nullptr);
  myTextCommand->setChecked(ScoreUtils::findByPosition(system.getTextItems(), position) != nullptr);
  ...
  myRemoveDotCommand->setEnabled(
    pos && (pos->hasProperty(Position::Dotted) || pos->hasProperty(Position::DoubleDotted)));
  ...
  const bool isAlterationOfPace =
    (tempoMarker && tempoMarker->getMarkerType() == TempoMarker::AlterationOfPace);
  ...
  myDirectionCommand->setChecked(ScoreUtils::findByPosition(system.getDirections(), position) != nullptr);
  ...
  myHammerPullCommand->setChecked(note && note->hasProperty(Note::HammerOnOrPullOff));

  updateNoteProperty(myHammerOnFromNowhereCommand, note, Note::HammerOnFromNowhere);
  ...
  myArtificialHarmonicCommand->setChecked(note && note->hasArtificialHarmonic());
  ...
  updateNoteProperty(mySlideIntoFromAboveCommand, note, Note::SlideIntoFromAbove);
  updateNoteProperty(mySlideIntoFromBelowCommand, note, Note::SlideIntoFromBelow);
  updateNoteProperty(myShiftSlideCommand, note, Note::ShiftSlide);
  updateNoteProperty(myLegatoSlideCommand, note, Note::LegatoSlide);
  updateNoteProperty(mySlideOutOfDownwardsCommand, note, Note::SlideOutOfDownwards);
  updateNoteProperty(mySlideOutOfUpwardsCommand, note, Note::SlideOutOfUpwards);
  ...
  updatePositionProperty(myPickStrokeDownCommand, pos, Position::PickStrokeDown);

  myPlayerChangeCommand->setChecked(ScoreUtils::findByPosition(system.getPlayerChanges(), position) !=
                                    nullptr);
}

void TheTabEditor::enableEditing(bool enable)
{
  QList<QMenu*> menuList;
  menuList << myPositionMenu << myPositionSectionMenu << myPositionStaffMenu << myTextMenu << mySectionMenu
           << myLineSpacingMenu << myNotesMenu << myOctaveMenu << myRestsMenu << myMusicSymbolsMenu
...
void TheTabEditor::editRest(Position::DurationType duration)
{
  ScoreLocation& location = getLocation();
  const Position* pos = location.getPosition();
  ...
  if (pos && pos->isRest()) {
    if (pos->getDurationType() == duration) {
      ...
      myUndoManager->push(new EditNoteDuration(location, duration, true), location.getSystemIndex());
    }
  } else {
    myUndoManager->push(new AddRest(location, duration), location.getSystemIndex());
  }
...
void TheTabEditor::rewindPlaybackToStart()
{
...
void TheTabEditor::stopPlayback()
{
  assert(myIsPlaying);
...
void TheTabEditor::editKeySignature(const ScoreLocation& keyLocation)
{
  ...
  if (dialog.exec() == QDialog::Accepted) {
    myUndoManager->push(new EditKeySignature(location, dialog.getNewKey()), UndoManager::AFFECTS_ALL_SYSTEMS);
  }
}
...
void TheTabEditor::editTimeSignature(const ScoreLocation& timeLocation)
{
  ...
  if (dialog.exec() == QDialog::Accepted) {
    myUndoManager->push(new EditTimeSignature(location, dialog.getTimeSignature()),
                        UndoManager::AFFECTS_ALL_SYSTEMS);
  }
}
...
void TheTabEditor::editBarline(const ScoreLocation& barLocation)
{
  ...
  Barline* barline = ScoreUtils::findByPosition(system.getBarlines(), location.getPositionIndex());

  if (barline) {
    const bool isStartBar = *barline == system.getBarlines().front();
    const bool isEndBar = *barline == system.getBarlines().back();

    BarlineDialog dialog(this, barline->getBarType(), barline->getRepeatCount(), isStartBar, isEndBar);
    if (dialog.exec() == QDialog::Accepted) {
      myUndoManager->push(new EditBarline(location, dialog.getBarType(), dialog.getRepeatCount()),
                          location.getSystemIndex());
    }
  } else {
    BarlineDialog dialog(this, Barline::SingleBar, Barline::MIN_REPEAT_COUNT, false, false);
    if (dialog.exec() == QDialog::Accepted) {
      Barline bar(location.getPositionIndex(), dialog.getBarType(), dialog.getRepeatCount());
      myUndoManager->push(new AddBarline(location, bar), location.getSystemIndex());
    }
  }
}

void TheTabEditor::editClef(int system, int staff)
{
  ScoreLocation location = getLocation();
  ...
  myUndoManager->push(new EditStaff(location, newClef, currentStaff.getStringCount()),
                      location.getSystemIndex());
}

void TheTabEditor::editSimplePositionProperty(Command* command, Position::SimpleProperty property)
{
  ScoreLocation& location = getLocation();
  std::vector<Position*> selectedPositions = location.getSelectedPositions();
  ...
  if (enableProperty) {
    myUndoManager->push(new AddPositionProperty(location, property, command->text()),
                        location.getSystemIndex());
  } else {
    myUndoManager->push(new RemovePositionProperty(location, property, command->text()),
                        location.getSystemIndex());
  }
}

void TheTabEditor::editSimpleNoteProperty(Command* command, Note::SimpleProperty property)
{
  ScoreLocation& location = getLocation();
  ...
  if (enableProperty) {
    myUndoManager->push(new AddNoteProperty(location, property, command->text()), location.getSystemIndex());
  } else {
    myUndoManager->push(new RemoveNoteProperty(location, property, command->text()),
                        location.getSystemIndex());
  }
}

void TheTabEditor::insertSystem(int index)
{
  myUndoManager->push(new AddSystem(getLocation().getScore(), index), UndoManager::AFFECTS_ALL_SYSTEMS);
}

void TheTabEditor::insertStaff(int index)
{
  ...
  if (dialog.exec() == QDialog::Accepted) {
    ...
    myUndoManager->push(new AddStaff(getLocation(), staff, index), getLocation().getSystemIndex());
  }
}

void TheTabEditor::editStaff(int system, int staff)
{
  ScoreLocation location = getLocation();
  location.setSystemIndex(system);
  location.setStaffIndex(staff);
  ...
  if (dialog.exec() == QDialog::Accepted) {
    myUndoManager->push(new EditStaff(location, dialog.getClefType(), dialog.getStringCount()),
                        UndoManager::AFFECTS_ALL_SYSTEMS);
  }
}

void TheTabEditor::adjustLineSpacing(int amount)
{
  myUndoManager->push(new AdjustLineSpacing(getLocation().getScore(), amount),
                      UndoManager::AFFECTS_ALL_SYSTEMS);
}

ScoreArea* TheTabEditor::getScoreArea()
{
  return dynamic_cast<ScoreArea*>(myTabWidget->currentWidget());
}

Caret& TheTabEditor::getCaret()
{
  return myDocumentManager->getCurrentDocument().getCaret();
}

ScoreLocation& TheTabEditor::getLocation()
{
