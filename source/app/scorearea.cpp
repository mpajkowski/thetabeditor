...
void ScoreArea::renderDocument(const Document& document)
{
  ...
  myCaretPainter = new CaretPainter(document.getCaret(), document.getViewOptions());
  ...
  for (int i = 0; i < num_threads; ++i) {
    ...
    const int right = (i == num_threads - 1) ? myRenderedSystems.size() : (i + 1) * work_size;
...
void ScoreArea::redrawSystem(int index)
{
  ...
  double height = 0;
  if (index > 0) {
    height = myRenderedSystems.at(index - 1)->sceneBoundingRect().bottom() + SYSTEM_SPACING;
  }
...
void ScoreArea::print(QPrinter& printer)
{
  ...
  QRectF target_rect(0, 0, painter.device()->width(), painter.device()->height());
  ...
  for (int i = 0, n = items.length(); i < n; ++i) {
    ...
    const float ratio =
      std::min(target_rect.width() / source_rect.width(), target_rect.height() / source_rect.height());

    if (i > 0) {
      const QGraphicsItem* prev_item = items[i - 1];
      const double spacing = source_rect.y() - prev_item->sceneBoundingRect().bottom();
