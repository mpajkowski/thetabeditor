...
TuningDictionaryDialog::TuningDictionaryDialog(QWidget* parent, TuningDictionary& dictionary)
  : QDialog(parent)
  ...
  , myDictionary(dictionary)
{
  ...
  ui->tuningsList->setHeaderLabels({ tr("Name"), tr("Tuning (Low to High)") });
  ...
  connect(ui->stringsComboBox, SIGNAL(currentIndexChanged(int)), this, SLOT(onNumStringsChanged(int)));
  ...
  connect(ui->editTuningButton, SIGNAL(clicked()), this, SLOT(onEditTuning()));
  connect(ui->deleteTuningButton, SIGNAL(clicked()), this, SLOT(onDeleteTuning()));
...
void TuningDictionaryDialog::onNumStringsChanged(int index)
{
  ...
  for (Tuning* tuning : tunings) {
    auto item =
      new QTreeWidgetItem(QStringList({ QString::fromStdString(tuning->getName()),
...
void TuningDictionaryDialog::onCurrentTuningChanged(QTreeWidgetItem* current, QTreeWidgetItem*)
{
...
{
  return ui->tuningsList->currentItem()->data(0, Qt::UserRole).value<Tuning*>();
}
