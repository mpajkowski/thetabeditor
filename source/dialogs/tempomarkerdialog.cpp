  ...
  , myTripletFeelTypes(new QButtonGroup(this))
{
  ...
  myBeatTypes->addButton(ui->dottedNote32Button, TempoMarker::ThirtySecondDotted);
  ...
  myListessoBeatTypes->addButton(ui->listessoDottedNote2Button, TempoMarker::HalfDotted);
  myListessoBeatTypes->addButton(ui->listessoNote4Button, TempoMarker::Quarter);
  myListessoBeatTypes->addButton(ui->listessoDottedNote4Button, TempoMarker::QuarterDotted);
  myListessoBeatTypes->addButton(ui->listessoNote8Button, TempoMarker::Eighth);
  myListessoBeatTypes->addButton(ui->listessoDottedNote8Button, TempoMarker::EighthDotted);
  myListessoBeatTypes->addButton(ui->listessoNote16Button, TempoMarker::Sixteenth);
  myListessoBeatTypes->addButton(ui->listessoDottedNote16Button, TempoMarker::SixteenthDotted);
  myListessoBeatTypes->addButton(ui->listessoNote32Button, TempoMarker::ThirtySecond);
  myListessoBeatTypes->addButton(ui->listessoDottedNote32Button, TempoMarker::ThirtySecondDotted);
  ui->listessoNote2Button->setChecked(true);

  // Prevent triplet feel types from being selected at once.
  myTripletFeelTypes->addButton(ui->tripletFeelNoneCheckBox, TempoMarker::NoTripletFeel);
  myTripletFeelTypes->addButton(ui->tripletFeel8thCheckBox, TempoMarker::TripletFeelEighth);
  myTripletFeelTypes->addButton(ui->tripletFeel8thOffCheckBox, TempoMarker::TripletFeelEighthOff);
  myTripletFeelTypes->addButton(ui->tripletFeel16thCheckBox, TempoMarker::TripletFeelSixteenth);
  myTripletFeelTypes->addButton(ui->tripletFeel16thOffCheckBox, TempoMarker::TripletFeelSixteenthOff);
  ui->tripletFeelNoneCheckBox->setChecked(true);

  connect(ui->enableListessoCheckBox, SIGNAL(clicked(bool)), this, SLOT(onListessoChanged(bool)));
  ui->enableListessoCheckBox->setChecked(false);
  onListessoChanged(false);

  connect(
    ui->showMetronomeMarkerCheckBox, SIGNAL(clicked(bool)), this, SLOT(onShowMetronomeMarkerChanged(bool)));
...
TempoMarker TempoMarkerDialog::getTempoMarker() const
{
  ...
  marker.setBeatType(static_cast<TempoMarker::BeatType>(myBeatTypes->checkedId()));
  marker.setListessoBeatType(static_cast<TempoMarker::BeatType>(myListessoBeatTypes->checkedId()));
  marker.setTripletFeel(static_cast<TempoMarker::TripletFeelType>(myTripletFeelTypes->checkedId()));
