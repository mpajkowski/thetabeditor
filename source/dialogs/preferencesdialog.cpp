  ...
  for (size_t i = 0; i < device.getApiCount(); ++i) {
    for (unsigned int j = 0; j < device.getPortCount(i); ++j) {
      ...
      ui->midiPortComboBox->addItem(
        ...
        QVariant::fromValue(std::make_pair(static_cast<int>(i), static_cast<int>(j))));
    }
  }
...
void PreferencesDialog::loadCurrentSettings()
{
  ...
  if (api < device.getApiCount() && port < device.getPortCount(api)) {
    ui->midiPortComboBox->setCurrentIndex(
      ui->midiPortComboBox->findText(QString::fromStdString(device.getPortName(api, port))));
  }

  ui->vibratoStrengthSpinBox->setValue(settings->get(Settings::MidiVibratoLevel));

  ui->wideVibratoStrengthSpinBox->setValue(settings->get(Settings::MidiWideVibratoLevel));

  ui->metronomeEnabledCheckBox->setChecked(settings->get(Settings::MetronomeEnabled));

  ui->metronomePresetComboBox->setCurrentIndex(settings->get(Settings::MetronomePreset));

  ui->strongAccentVolumeSpinBox->setValue(settings->get(Settings::MetronomeStrongAccent));

  ui->weakAccentVolumeSpinBox->setValue(settings->get(Settings::MetronomeWeakAccent));

  ui->countInEnabledCheckBox->setChecked(settings->get(Settings::CountInEnabled));

  ui->countInPresetComboBox->setCurrentIndex(settings->get(Settings::CountInPreset));

  ui->countInVolumeSpinBox->setValue(settings->get(Settings::CountInVolume));

  ui->openInNewWindowCheckBox->setChecked(settings->get(Settings::OpenFilesInNewWindow));

  ui->defaultInstrumentNameLineEdit->setText(
    QString::fromStdString(settings->get(Settings::DefaultInstrumentName)));
  ui->defaultPresetComboBox->setCurrentIndex(settings->get(Settings::DefaultInstrumentPreset));
  ...
  ui->defaultTuningClickButton->setText(
    QString::fromStdString(boost::lexical_cast<std::string>(myDefaultTuning)));
...
void PreferencesDialog::accept()
{
  ...
  MidiApiAndPort apiAndPort =
    ui->midiPortComboBox->itemData(ui->midiPortComboBox->currentIndex()).value<MidiApiAndPort>();
  settings->set(Settings::MidiApi, apiAndPort.first);
  settings->set(Settings::MidiPort, apiAndPort.second);

  settings->set(Settings::MidiVibratoLevel, ui->vibratoStrengthSpinBox->value());

  settings->set(Settings::MidiWideVibratoLevel, ui->wideVibratoStrengthSpinBox->value());

  settings->set(Settings::MetronomeEnabled, ui->metronomeEnabledCheckBox->isChecked());

  settings->set(Settings::MetronomePreset, ui->metronomePresetComboBox->currentIndex());

  settings->set(Settings::MetronomeStrongAccent, ui->strongAccentVolumeSpinBox->value());

  settings->set(Settings::MetronomeWeakAccent, ui->weakAccentVolumeSpinBox->value());

  settings->set(Settings::CountInEnabled, ui->countInEnabledCheckBox->isChecked());

  settings->set(Settings::CountInPreset, ui->countInPresetComboBox->currentIndex());

  settings->set(Settings::CountInVolume, ui->countInVolumeSpinBox->value());

  settings->set(Settings::OpenFilesInNewWindow, ui->openInNewWindowCheckBox->isChecked());

  settings->set(Settings::DefaultInstrumentName, ui->defaultInstrumentNameLineEdit->text().toStdString());

  settings->set(Settings::DefaultInstrumentPreset, ui->defaultPresetComboBox->currentIndex());
