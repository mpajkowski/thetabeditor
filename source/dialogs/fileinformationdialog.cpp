  ...
  , ui(new Ui::FileInformationDialog)
{
  ...
  ui->songTypeButtonGroup->setId(ui->songButton, static_cast<int>(ScoreInfo::ScoreType::Song));
  ui->songTypeButtonGroup->setId(ui->lessonButton, static_cast<int>(ScoreInfo::ScoreType::Lesson));

  connect(ui->songTypeButtonGroup,
          SIGNAL(buttonClicked(QAbstractButton*)),
          this,
          SLOT(handleSongTypeButtonClick(QAbstractButton*)));

  connect(ui->releaseTypeList, SIGNAL(currentRowChanged(int)), this, SLOT(handleReleaseTypeChanged(int)));

  connect(ui->traditionalSongValue, SIGNAL(toggled(bool)), this, SLOT(handleAuthorTypeChanged(bool)));
  ...
  if (info.getScoreType() == ScoreInfo::ScoreType::Song) {
    ...
    if (song.isAudioRelease()) {
      ...
      const SongData::AudioReleaseInfo& audioRelease = song.getAudioReleaseInfo();

      ui->albumTitleValue->setText(QString::fromStdString(audioRelease.getTitle()));
      ui->albumTypeValue->setCurrentIndex(static_cast<int>(audioRelease.getReleaseType()));
      ...
      const SongData::VideoReleaseInfo& videoRelease = song.getVideoReleaseInfo();

      ui->videoTitleValue->setText(QString::fromStdString(videoRelease.getTitle()));
      ...
      ui->bootlegTitleValue->setText(QString::fromStdString(bootleg.getTitle()));

      const boost::gregorian::date date = bootleg.getDate();
      ui->bootlegDateValue->setDate(QDate(date.year(), date.month(), date.day()));
    ...
    if (!song.isTraditionalAuthor()) {
      ...
      ui->composerValue->setText(QString::fromStdString(authorInfo.getComposer()));
      ui->lyricistValue->setText(QString::fromStdString(authorInfo.getLyricist()));
    }

    ui->arrangerValue->setText(QString::fromStdString(song.getArranger()));
    ui->transcriberValue->setText(QString::fromStdString(song.getTranscriber()));
    ui->songCopyrightValue->setText(QString::fromStdString(song.getCopyright()));

    // Initialize performance notes.
    ui->notesValue->setPlainText(QString::fromStdString(song.getPerformanceNotes()));
    ...
    ui->lessonStyleValue->setCurrentIndex(static_cast<int>(lesson.getMusicStyle()));

    QAbstractButton* levelButton =
      ui->lessonLevelButtonGroup->button(static_cast<int>(lesson.getDifficultyLevel()));
    Q_ASSERT(levelButton != NULL);
    levelButton->click();

    ui->lessonAuthorValue->setText(QString::fromStdString(lesson.getAuthor()));
    ui->lessonCopyrightValue->setText(QString::fromStdString(lesson.getCopyright()));
  }
  ...
  ui->fileSizeValue->setText(tr("%1 bytes").arg(locale.toString(fileInfo.size())));
...
ScoreInfo FileInformationDialog::getScoreInfo() const
{
  ...
  if (ui->songTypeStack->currentIndex() == 0) // Song
  {
    ...
    switch (ui->releaseInfoStack->currentIndex()) {
      case 0:
        song.setAudioReleaseInfo(SongData::AudioReleaseInfo(
          static_cast<SongData::AudioReleaseInfo::ReleaseType>(ui->albumTypeValue->currentIndex()),
          ui->albumTitleValue->text().toStdString(),
          ui->albumYearValue->value(),
          ui->audioLiveRecordingValue->isChecked()));
        break;
      case 1:
        song.setVideoReleaseInfo(SongData::VideoReleaseInfo(ui->videoTitleValue->text().toStdString(),
                                                            ui->videoLiveRecordingValue->isChecked()));
        break;
      case 2: {
        const QDate date = ui->bootlegDateValue->date();
        song.setBootlegInfo(
          SongData::BootlegInfo(ui->bootlegTitleValue->text().toStdString(),
    ...
    if (ui->traditionalSongValue->isChecked())
      ...
      song.setAuthorInfo(SongData::AuthorInfo(ui->composerValue->text().toStdString(),
  ...
  } else // Lesson
  {
    ...
    lesson.setMusicStyle(static_cast<LessonData::MusicStyle>(ui->lessonStyleValue->currentIndex()));
    lesson.setDifficultyLevel(
      static_cast<LessonData::DifficultyLevel>(ui->lessonLevelButtonGroup->checkedId()));
