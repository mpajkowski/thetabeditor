  ...
  , ui(new Ui::ChordNameDialog)
{
  ...
  connect(myTonicKeys, SIGNAL(buttonClicked(int)), this, SLOT(onTonicChanged()));
  ...
  connect(ui->formulaListWidget, SIGNAL(currentRowChanged(int)), this, SLOT(updateState()));
...
void ChordNameDialog::updateState()
{
  ...
  myChord.setFormula(static_cast<ChordName::Formula>(ui->formulaListWidget->currentRow()));
  ...
  myChord.setModification(ChordName::Added11th, ui->add11CheckBox->isChecked());

  myChord.setModification(ChordName::Extended9th, ui->extend9CheckBox->isChecked());
  myChord.setModification(ChordName::Extended11th, ui->extend11CheckBox->isChecked());
  myChord.setModification(ChordName::Extended13th, ui->extend13CheckBox->isChecked());

  myChord.setModification(ChordName::Flatted5th, ui->flat5CheckBox->isChecked());
  myChord.setModification(ChordName::Raised5th, ui->raised5CheckBox->isChecked());
  myChord.setModification(ChordName::Flatted9th, ui->flat9CheckBox->isChecked());
  myChord.setModification(ChordName::Raised9th, ui->raised9CheckBox->isChecked());
  myChord.setModification(ChordName::Raised11th, ui->raised11CheckBox->isChecked());
  myChord.setModification(ChordName::Flatted13th, ui->flat13CheckBox->isChecked());
  myChord.setModification(ChordName::Suspended2nd, ui->sus2CheckBox->isChecked());
  myChord.setModification(ChordName::Suspended4th, ui->sus4CheckBox->isChecked());

  ui->previewLineEdit->setText(QString::fromStdString(boost::lexical_cast<std::string>(myChord)));
}
