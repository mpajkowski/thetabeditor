    ...
    for (const Barline& barline : system.getBarlines()) {
      if (barline.hasRehearsalSign()) {
        ...
        ui->rehearsalSignComboBox->addItem(
          QString("%1 -- %2")
            .arg(QString::fromStdString(sign.getLetters()), QString::fromStdString(sign.getDescription())),
...
ScoreLocation GoToRehearsalSignDialog::getLocation() const
{
  ...
  SystemAndPosition location = ui->rehearsalSignComboBox->itemData(index).value<SystemAndPosition>();
