    ...
    : QDialog(parent), ui(new Ui::DirectionDialog), myCurrentSymbol(0)
{
    ...
    for (int i = DirectionSymbol::Coda; i < DirectionSymbol::NumSymbolTypes;
         ++i)
    {
    ...
    connect(ui->addDirectionButton, SIGNAL(clicked()), this,
            SLOT(onAddDirection()));
    connect(ui->removeDirectionButton, SIGNAL(clicked()), this,
            SLOT(onRemoveDirection()));
    connect(ui->directionComboBox, SIGNAL(currentIndexChanged(int)), this,
            SLOT(onSymbolIndexChanged(int)));
}
...
void DirectionDialog::accept()
{
    myDirection.getSymbols()[myCurrentSymbol] =
        DirectionSymbol(static_cast<DirectionSymbol::SymbolType>(
