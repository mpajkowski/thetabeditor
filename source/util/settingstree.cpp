...
struct JSONSerializer : public boost::static_visitor<void>
{
  ...
  void operator()(int x) { myWriter.Int(x); }

  void operator()(bool x) { myWriter.Bool(x); }
...
boost::optional<SettingValue> SettingsTree::find(const std::string& key) const
{
...
void SettingsTree::loadFromJSON(std::istream& is)
{
  ...
  if (document.HasParseError()) {
    throw std::runtime_error("Parse error at offset " + std::to_string(document.GetErrorOffset()) + ": " +
