...
template<typename T>
typename T::pointer findByPosition(const boost::iterator_range<T>& range, int position)
{
  size_t n = range.size();
...
boost::filtered_range<InPositionRange, Range> findInRange(Range range, int left, int right)
{
...
void insertObject(std::vector<T>& objects, T const& obj)
{
  ...
  const bool needsSort = !objects.empty() && objects.back().getPosition() > obj.getPosition();
