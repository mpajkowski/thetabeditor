...
SystemLocation RepeatedSection::performRepeat(SystemLocation const& loc)
{
  ...
  if (firstAltEnding && *firstAltEnding == loc) {
    ...
    boost::optional<SystemLocation> nextAltEnding = findAlternateEnding(myActiveRepeat);
    if (nextAltEnding)
    ...
    for (Barline const& bar : system.getBarlines()) {
      ...
      if (nextBar) {
        for (AlternateEnding const& ending : ScoreUtils::findInRange(
          ...
          if (!repeats.empty()) {
            repeats.top().addAlternateEnding(system, systemIndex, ending);
          }
        }
      }
      ...
      if (!repeats.empty()) {
        ...
        if (activeRepeat.getAlternateEndingCount() && !activeRepeat.getRepeatEndBars().empty() &&
            activeRepeat.getAlternateEndingCount() >= activeRepeat.getTotalRepeatCount()) {
      ...
      // TODO - report unexpected repeat end bars.
        ...
        RepeatedSection& activeRepeat = repeats.top();
        activeRepeat.addRepeatEndBar(SystemLocation(systemIndex, bar.getPosition()), bar.getRepeatCount());
...
boost::iterator_range<RepeatIndexer::RepeatedSectionIterator> RepeatIndexer::getRepeats() const
{
