...
static void expandScore(Score& score, ExpandedBarList& expanded_bars)
{
  ...
  while (true) {
    ...
    const SystemLocation location(score_loc.getSystemIndex(), score_loc.getPositionIndex());
    const SystemLocation next_bar_loc(location.getSystem(), next_bar->getPosition());
    ...
    if (!ScoreUtils::findInRange(
           system.getAlternateEndings(), prev_bar->getPosition(), next_bar->getPosition() - 1)
           .empty()) {
    ...
    if (multibar_rest) {
      for (int i = multibar_rest->getMultiBarRestCount(); i > 0; --i) {
        expanded_bars.emplace_back(location,
                                   i != multibar_rest->getMultiBarRestCount(),
                                   i,
                                   *prev_bar,
                                   remaining_repeats,
    ...
    if (active_repeat) {
      ...
      if (new_loc != next_bar_loc) {
        ...
        continue;
      } else if (remaining_repeats == 1 && next_bar->getBarType() == Barline::RepeatEnd) {
        remaining_repeats = 0;
      }
    }
...
static int importNotes(ScoreLocation& dest_loc,
                       ScoreLocation& src_loc,
                       bool is_bass,
                       ...
                       std::function<int(ScoreLocation&, ScoreLocation&)> action)
{
  ...
  for (unsigned int i = 0; i < src_system.getStaves().size(); ++i) {
    ...
    assert(src_loc.getStaff().getStringCount() == dest_loc.getStaff().getStringCount());
  ...
  int right)
{
  ...
  for (Symbol const& src_symbol : ScoreUtils::findInRange(src_symbols, left, right - 1)) {
    ...
    if (filled_positions.find(symbol.getPosition()) != filled_positions.end()) {
      continue;
    }
                               ...
                               ExpandedBar const& src_bar)
{
  ...
  if (!src_bar.isExpanded()) {
    copySymbols(src_system.getTempoMarkers(),
                dest_system,
                dest_system.getTempoMarkers(),
                &System::insertTempoMarker,
                offset,
                left,
                right);

    copySymbols(src_system.getTextItems(),
                dest_system,
                dest_system.getTextItems(),
  ...
  copySymbols(
    src_system.getChords(), dest_system, dest_system.getChords(), &System::insertChord, offset, left, right);

  if (src_bar.isAlternateEnding()) {
    copySymbols(src_system.getAlternateEndings(),
                dest_system,
                dest_system.getAlternateEndings(),
                &System::insertAlternateEnding,
                offset,
                left,
                right);
  }
                       ...
                       bool is_bass)
{
  ...
  if (src_bar.getMultiBarRestCount() > 0) {
    auto insert_rest = std::bind(
      insertMultiBarRest, std::placeholders::_1, std::placeholders::_2, src_bar.getMultiBarRestCount());
    return importNotes(dest_loc, src_loc, is_bass, src_bar.isExpanded(), num_guitar_staves, insert_rest);
  } else {
    return importNotes(dest_loc, src_loc, is_bass, src_bar.isExpanded(), num_guitar_staves, copyNotes);
  }
}
                                            ...
                                            ExpandedBarList::const_iterator end_src_bar)
{
  ...
  auto changes = ScoreUtils::findInRange(src_loc.getSystem().getPlayerChanges(), left, right - 1);
...
{
  System& dest_system = dest_loc.getSystem();
  const PlayerChange* guitar_change = findPlayerChange(dest_loc, guitar_loc, guitar_bar, end_guitar_bar);
  const PlayerChange* bass_change = findPlayerChange(dest_loc, bass_loc, bass_bar, end_bass_bar);
  ...
  if (guitar_change || bass_change ||
    ...
    if (!guitar_change && guitar_bar != end_guitar_bar) {
      ...
      guitar_change = ScoreUtils::getCurrentPlayers(
        guitar_loc.getScore(), guitar_loc.getSystemIndex(), guitar_loc.getPositionIndex());
    }
    ...
    if (!bass_change && bass_bar != end_bass_bar) {
      ...
      bass_change = ScoreUtils::getCurrentPlayers(
        bass_loc.getScore(), bass_loc.getSystemIndex(), bass_loc.getPositionIndex());
    }

    // Merge in data from only the active staves.
    if (guitar_change) {
      for (int i = 0; i < num_guitar_staves; ++i) {
        for (auto player : guitar_change->getActivePlayers(i)) {
    ...
    if (bass_change) {
      for (unsigned int i = 0; i < bass_loc.getSystem().getStaves().size(); ++i) {
        for (auto player : bass_change->getActivePlayers(i)) {
          change.insertActivePlayer(num_guitar_staves + i,
                                    guitar_loc.getScore().getPlayers().size() + player);
        }
      }
    }
...
static bool areStavesIncompatible(ScoreLocation const& dest_loc,
                                  ...
                                  int num_guitar_staves)
{
  return areStavesIncompatible(dest_loc, guitar_caret, guitar_bar, end_guitar_bar, 0, num_guitar_staves) ||
         areStavesIncompatible(dest_loc,
                               bass_caret,
                               bass_bar,
                               end_bass_bar,
                               num_guitar_staves,
                          ...
                          ExpandedBarList const& bass_bars)
{
  ...
  while (guitar_bar != end_guitar_bar || bass_bar != end_bass_bar) {
    ...
    const ExpandedBar* prev_bar = (guitar_bar != guitar_bars.begin()) ? &*boost::prior(current_bar) : nullptr;
    ...
    if (dest_loc.getPositionIndex() > 0) {
      ...
      if (prev_bar && prev_bar->isRepeatEnd() &&
        ...
        dest_system.insertBarline(
          Barline(dest_loc.getPositionIndex(), Barline::RepeatEnd, prev_bar->getRemainingRepeats()));
        dest_caret.moveHorizontal(1);
      }

      dest_system.insertBarline(Barline(dest_loc.getPositionIndex(), Barline::SingleBar));
    }

    // Set the barline's properties, key signature, etc.
    Barline* barline = ScoreUtils::findByPosition(dest_system.getBarlines(), dest_loc.getPositionIndex());
    *barline = current_bar->getStartBar();
    ...
    if (guitar_bar != end_guitar_bar) {
      bar_length =
        std::max(bar_length, copyContent(dest_loc, num_guitar_staves, guitar_caret, *guitar_bar, false));
    }
    if (bass_bar != end_bass_bar) {
      bar_length =
        std::max(bar_length, copyContent(dest_loc, num_guitar_staves, bass_caret, *bass_bar, true));
    }

    mergePlayerChanges(dest_loc,
                       guitar_loc,
                       bass_loc,
                       guitar_bar,
                       end_guitar_bar,
    ...
    const int next_bar_pos = dest_caret.getLocation().getPositionIndex() + bar_length;

    bool need_new_system = next_bar_pos > thePositionLimit;
    need_new_system |= areStavesIncompatible(dest_loc,
                                             guitar_caret,
                                             bass_caret,
                                             guitar_bar,
                                             end_guitar_bar,
    ...
    const bool finishing = (guitar_bar == end_guitar_bar && bass_bar == end_bass_bar);
...
static ExpandedBarList::iterator mergeMultiBarRest(ExpandedBarList& bars,
...
static void mergeMultiBarRests(ExpandedBarList& guitar_bars, ExpandedBarList& bass_bars)
{
  auto guitar_bar = guitar_bars.begin();
  ...
  while (guitar_bar != guitar_end_bar && bass_bar != bass_end_bar) {
    if (guitar_bar->getMultiBarRestCount() > 0 && bass_bar->getMultiBarRestCount() > 0) {
      const int count = std::min(guitar_bar->getMultiBarRestCount(), bass_bar->getMultiBarRestCount());
  ...
  while (guitar_bar != guitar_end_bar) {
    if (guitar_bar->getMultiBarRestCount() > 0) {
      guitar_bar = mergeMultiBarRest(guitar_bars, guitar_bar, guitar_bar->getMultiBarRestCount());
    } else
      ++guitar_bar;
  }
  while (bass_bar != bass_end_bar) {
    if (bass_bar->getMultiBarRestCount() > 0) {
      bass_bar = mergeMultiBarRest(bass_bars, bass_bar, bass_bar->getMultiBarRestCount());
    } else
      ++bass_bar;
  }
}

static ExpandedBarList::iterator clearRepeatedSection(ExpandedBarList::iterator bar,
                                                      ExpandedBarList::iterator end_bar)
{
...
static ExpandedBarList::iterator collapseRepeatedSection(ExpandedBarList& bars,
                                                         ExpandedBarList::iterator bar,
                                                         int num_repeats)
{
  ...
  for (int i = first_repeat - 1; i > first_repeat - num_repeats; --i) {
    while (bar != bars.end() && bar->getRemainingRepeats() == i) {
      if (bar->isAlternateEnding() && known_bars.find(bar->getLocation()) == known_bars.end()) {
        known_bars.insert(bar->getLocation());
        ++bar;
      } else
        bar = bars.erase(bar);
...
static void trivialMergeRepeats(ExpandedBarList& bars, ExpandedBarList::iterator bar)
{
  while (bar != bars.end()) {
...
static void mergeRepeats(ExpandedBarList& guitar_bars, ExpandedBarList& bass_bars)
{
  auto guitar_bar = guitar_bars.begin();
  auto guitar_end_bar = guitar_bars.end();
  auto bass_bar = bass_bars.begin();
  ...
  while (guitar_bar != guitar_end_bar && bass_bar != bass_end_bar) {
    if (guitar_bar->getStartBar().getBarType() == Barline::RepeatStart &&
      ...
      const int num_repeats = std::min(guitar_bar->getRemainingRepeats(), bass_bar->getRemainingRepeats());
      ...
      if (guitar_bar->isRepeatEnd() && bass_bar->isRepeatEnd()) {
        guitar_bar = collapseRepeatedSection(guitar_bars, guitar_section_start, num_repeats);
        bass_bar = collapseRepeatedSection(bass_bars, bass_section_start, num_repeats);
      } else {
...
void ScoreMerger::merge(Score& dest_score, Score& guitar_score, Score& bass_score)
{
