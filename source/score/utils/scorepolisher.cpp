...
public:
  bool operator<(const TimeStamp& other) const
  {
    if (myTime == other.myTime) {
      ...
      // note.
      return myGraceNoteNumber.get_value_or(std::numeric_limits<int>::max()) <
             other.myGraceNoteNumber.get_value_or(std::numeric_limits<int>::max());
    } else
      return myTime < other.myTime;
  }

  void advance(const boost::rational<int>& duration) { myTime += duration; }

  void setGraceNoteNumber(boost::optional<int> count) { myGraceNoteNumber = count; }
...
static void shiftAllItemsAtPosition(System& system,
                                    Staff& staff,
                                    Voice& voice,
                                    int currentPosition,
                                    int newPosition,
                                    std::unordered_set<const void*>& knownItems)
{
  shiftItemsAtPosition(voice.getIrregularGroupings(), currentPosition, newPosition, knownItems);
  shiftItemsAtPosition(staff.getDynamics(), currentPosition, newPosition, knownItems);
  shiftItemsAtPosition(system.getTextItems(), currentPosition, newPosition, knownItems);
  shiftItemsAtPosition(system.getChords(), currentPosition, newPosition, knownItems);
  shiftItemsAtPosition(system.getTempoMarkers(), currentPosition, newPosition, knownItems);
  shiftItemsAtPosition(system.getDirections(), currentPosition, newPosition, knownItems);
  shiftItemsAtPosition(system.getPlayerChanges(), currentPosition, newPosition, knownItems);
...
void ScoreUtils::polishSystem(System& system)
{
  ...
  for (Barline& leftBar : system.getBarlines()) {
    ...
    for (const Staff& staff : system.getStaves()) {
      for (const Voice& voice : staff.getVoices()) {
        ...
        for (const Position& position :
          ...
          computeTimestampPosition(timestamp, currentPosition, timestampPositions);
          boost::rational<int> duration = VoiceUtils::getDurationTime(voice, position);

          currentPosition = timestampPositions[timestamp] + getDefaultNoteSpacing(duration);
    ...
    // Adjust!
    const int startPos = (leftBar.getPosition() == 0) ? 0 : leftBar.getPosition() + 1;
    const int oldEndPos = rightBar->getPosition();
    const int endPos = startPos + maxPosition;
    std::unordered_set<const void*> knownItems;

    if (endPos > oldEndPos) {
      SystemUtils::shift(system, rightBar->getPosition(), endPos - rightBar->getPosition());
    } else {
      for (Staff& staff : system.getStaves()) {
        for (Voice& voice : staff.getVoices()) {
          shiftAllItemsAtPosition(system, staff, voice, oldEndPos, endPos, knownItems);
        }
      }
    ...
    for (Staff& staff : system.getStaves()) {
      for (Voice& voice : staff.getVoices()) {
        for (Position& pos :
             ScoreUtils::findInRange(voice.getPositions(), leftBar.getPosition(), oldEndPos)) {
          ...
          const int newPosition = startPos + timestampPositions[timestamp];
          ...
          shiftAllItemsAtPosition(system, staff, voice, currentPosition, newPosition, knownItems);
