...
const std::map<int, int> theHarmonicOffsets = { { 2, 38 },  { 3, 31 },  { 4, 28 },  { 5, 24 },
                                                { 7, 19 },  { 9, 28 },  { 10, 38 }, { 12, 12 },
                                                { 14, 38 }, { 15, 34 }, { 16, 28 }, { 17, 36 },
...
bool Note::operator==(Note const& other) const
{
  return myString == other.myString && myFretNumber == other.myFretNumber &&
         ...
         myArtificialHarmonic == other.myArtificialHarmonic && myBend == other.myBend;
}
...
std::ostream& operator<<(std::ostream& os, Note const& note)
{
  ...
  std::string brackets = "";
  if (note.hasTappedHarmonic() || note.hasProperty(Note::GhostNote) || note.hasTrill()) {
    brackets = "()";
